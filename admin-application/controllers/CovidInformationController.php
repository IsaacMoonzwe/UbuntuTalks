...
class CovidInformationController extends AdminBaseController
{
    ...
    {
        $srch = CovidInformation::getSearchObject($this->adminLangId, false);
        $srch->addOrder('covid_information_active', 'DESC');
    ...
    public function form($epage_id = 0)
    {
        ...
        if (0 < $epage_id) {
            $data = CovidInformation::getAttributesById($epage_id, array('epage_id', 'covid_information_identifier', 'covid_information_active'));
            if ($data === false) {
    ...
    public function setup()
    {
        ...
        $extraPage = new CovidInformation($epage_id);
        ...
        foreach ($languages as $langId => $langName) {
            if (!$row = CovidInformation::getAttributesByLangId($langId, $epage_id)) {
                $newTabLangId = $langId;
                break;
            }
        }
    ...
    public function langForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $epageData = CovidInformation::getAttributesById($epage_id);
        $blockLangFrm = $this->getLangForm($epage_id, $lang_id);
        $langData = CovidInformation::getAttributesByLangId($lang_id, $epage_id);
        if ($langData) {
    ...
    public function langSetup()
    {
        ...
        $data = array(
            ...
            'covid_information_label' => $post['covid_information_label'],
            'covid_information_content' => $post['covid_information_content'],
        );
        $epageObj = new CovidInformation($epage_id);
    ...
    public function changeStatus()
    {
        ...
        $contentBlockData = CovidInformation::getAttributesById($epageId, array('covid_information_active'));
        if (!$contentBlockData) {
            Message::addErrorMessage($this->str_invalid_request);
            FatUtility::dieWithError(Message::getHtml());
        }
        $status = ($contentBlockData['covid_information_active'] == applicationConstants::ACTIVE) ? applicationConstants::INACTIVE : applicationConstants::ACTIVE;
    ...
    private function updateEPageStatus($epageId, $status)
    {
        ...
        $EPageObj = new CovidInformation($epageId);
    ...
    private function getForm($epage_id = 0, $langId = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Identifier', $this->adminLangId), 'covid_information_identifier');

        $activeInactiveArr = applicationConstants::getActiveInactiveArr($langId);
        $frm->addSelectBox(Label::getLabel('LBL_Status', $this->adminLangId), 'covid_information_active', $activeInactiveArr, '', array(), '');
    ...
    private function getLangForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Title', $this->adminLangId), 'covid_information_label');
        $frm->addHtmlEditor(Label::getLabel('LBL_Page_Content', $this->adminLangId), 'covid_information_content');
