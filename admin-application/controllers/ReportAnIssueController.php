...
class ReportAnIssueController extends AdminBaseController
{
    ...
    public function purchasedLessonsSearch()
    {
        ...
        $srch->addMultipleFields([
            ...
            'slesson_added_on', 'sldetail_learner_id', 'op_lpackage_is_free_trial', 'order_is_paid',
        ...
        $ReportIssueSection->setPageNumber($page);
        $ReportIssueSection->setPageSize($pagesize);

        foreach ($records as $key => $value) { 
            $UserId = $value['sldetail_learner_id'];
            $ReportIssueSection = new SearchBase('tbl_report_comments');
            $ReportIssueSection->addCondition('user_id', ' = ', $UserId);
            $ReportIssueSection->addMultipleFields(['*','COUNT(user_id) AS total_record','SUM(report_comments_status) As count_status']);
            ...
            $value['Reports'] = $ReportIssueCategoriesList;
            $records[$key] = $value;
            $array[$key] = $ReportIssueCategoriesList;
        }
        
        $this->set("rawData", $rawData);
        $this->set("arr_listing", $records);
        $this->set('pageCount', $ReportIssueSection->pages());
        $this->set('page', $page);
    ...
    public function viewSchedules($status = "all", $orderId = null)
    {
        ...
        $newClass=new SearchBase('tbl_report_comments');
    ...
    private function getAgendaForm($testimonialId)
    {
        ...
        $agendafrm->addTextarea(Label::getLabel('LBL_Event_Location', $this->adminLangId), 'report_comments_admin_information');
    ...
    public function reportDetail($slesson_id)
    {
        ...
        foreach ($ReportIssueCategoriesList as $key => $value) {
            $agendafrm = $this->getAgendaForm($value['report_comments_id']);
        }
        if (0 < $testimonialId) {
    ...
    public function agendasetup()
    {
        ...
        $res = EmailHandler::sendSmtpEmail($userData['credential_email'], 'Report Issue', $post['report_comments_admin_information'], '', '', $this->siteLangId, '', '');
