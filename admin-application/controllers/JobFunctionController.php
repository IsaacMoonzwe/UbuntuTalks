...
class JobFunctionController extends AdminBaseController
{
    ...
    {
        $srch = JobFunction::getSearchObject($this->adminLangId, false);
    ...
    public function form($testimonialId)
    {
        ...
        if (0 < $testimonialId) {
            $data = JobFunction::getAttributesById($testimonialId, [
                        'testimonial_id',
    ...
    public function setup()
    {
        ...
        $record = new JobFunction($testimonialId);
        ...
        if ($testimonialId > 0) {
            ...
            foreach ($languages as $langId => $langName) {
                if (!$row = JobFunction::getAttributesByLangId($langId, $testimonialId)) {
                    $newTabLangId = $langId;
                    break;
                }
            }
    ...
    public function langForm($testimonialId = 0, $lang_id = 0)
    {
        ...
        $langData = JobFunction::getAttributesByLangId($lang_id, $testimonialId);
        if ($langData) {
    ...
    public function langSetup()
    {
        ...
        $obj = new JobFunction($testimonialId);
    ...
    public function changeStatus()
    {
        ...
        $data = JobFunction::getAttributesById($testimonialId, ['testimonial_id', 'testimonial_active']);
        if ($data == false) {
    ...
    public function deleteRecord()
    {
        ...
        $testimonialObj = new JobFunction($testimonial_id);
        if (!$testimonialObj->canRecordMarkDelete($testimonial_id)) {
            Message::addErrorMessage($this->str_invalid_request_id);
            FatUtility::dieJsonError(Message::getHtml());
        }
        $testimonialObj->assignValues([JobFunction::tblFld('deleted') => 1]);
    ...
    private function getForm($testimonialId = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Job_Function_List_Name', $this->adminLangId), 'testimonial_user_name');
    ...
    private function getLangForm($testimonialId = 0, $lang_id = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Job_Function_Title', $this->adminLangId), 'testimonial_title');
        $frm->addTextarea(Label::getLabel('LBL_Job_Function_Text', $this->adminLangId), 'testimonial_text');
        $frm->addSubmitButton('', 'btn_submit', Label::getLabel('LBL_Save_Changes', $this->adminLangId));
        return $frm;
    }
