...
class EventContactController extends AdminBaseController
{
    ...
    {
        $srch = EventContact::getSearchObject($this->adminLangId, false);
        $srch->addOrder('event_contact_active', 'DESC');
    ...
    public function form($epage_id = 0)
    {
        ...
        if (0 < $epage_id) {
            $data = EventContact::getAttributesById($epage_id, array('epage_id', 'event_contact_identifier', 'event_contact_active'));
            if ($data === false) {
    ...
    public function setup()
    {
        ...
        $extraPage = new EventContact($epage_id);
        ...
        foreach ($languages as $langId => $langName) {
            if (!$row = EventContact::getAttributesByLangId($langId, $epage_id)) {
                $newTabLangId = $langId;
                break;
            }
        }
    ...
    public function langForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $epageData = EventContact::getAttributesById($epage_id);
        $blockLangFrm = $this->getLangForm($epage_id, $lang_id);
        $langData = EventContact::getAttributesByLangId($lang_id, $epage_id);
        if ($langData) {
    ...
    public function langSetup()
    {
        ...
        $data = array(
            ...
            'event_contact_label' => $post['event_contact_label'],
            'event_contact_content' => $post['event_contact_content'],
        );
        $epageObj = new EventContact($epage_id);
    ...
    public function changeStatus()
    {
        ...
        $contentBlockData = EventContact::getAttributesById($epageId, array('event_contact_active'));
        if (!$contentBlockData) {
            Message::addErrorMessage($this->str_invalid_request);
            FatUtility::dieWithError(Message::getHtml());
        }
        $status = ($contentBlockData['event_contact_active'] == applicationConstants::ACTIVE) ? applicationConstants::INACTIVE : applicationConstants::ACTIVE;
    ...
    private function updateEPageStatus($epageId, $status)
    {
        ...
        $EPageObj = new EventContact($epageId);
    ...
    private function getLangForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Title', $this->adminLangId), 'event_contact_label');
        $frm->addHtmlEditor(Label::getLabel('LBL_Page_Content', $this->adminLangId), 'event_contact_content');
