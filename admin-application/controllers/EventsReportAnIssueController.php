...
class EventsReportAnIssueController extends AdminBaseController
{
    ...
    public function calendarJsonData()
    {
        ...
        $srch->addCondition('sld.sldetail_learner_id', ' = ', EventUserAuthentication::getLoggedUserId());
    ...
    {
        $ReportIssueSection = new SearchBase('tbl_events_report_comments');
        $ReportIssueSection->addMultipleFields(['*', 'COUNT(user_id) AS total_record', 'SUM(events_report_comments_status) As count_status']);
        $ReportIssueSection->addGroupBy('user_id');
        ...
        foreach ($ReportIssueCategoriesList as $key => $value) {
            ...
            $ReportIssueCategoriesList[$key] = $value;
        }
        $this->set("arr_listing", $ReportIssueCategoriesList);
    ...
    public function setUpLessonSchedule()
    {
        ...
        // $getLessonDetailObj->addCondition('sldetail_learner_id', '=', EventUserAuthentication::getLoggedUserId());
        ...
        $userIds = [$teacher_id, EventUserAuthentication::getLoggedUserId()];
        ...
        if (ScheduledLesson::STATUS_SCHEDULED == $lessonDetail['sldetail_learner_status'] && $rescheduleReason) {
            $lessonResLogArr = [
                ...
                'lesreschlog_reschedule_by' => EventUserAuthentication::getLoggedUserId(),
            ...
            $lessonStsLog->addLog(ScheduledLesson::STATUS_SCHEDULED, User::USER_TYPE_LEANER, EventUserAuthentication::getLoggedUserId(), $rescheduleReason);
    ...
    public function isSlotTaken()
    {
        ...
        $db = FatApp::getDb();
        $userIds = [$teacherId, EventUserAuthentication::getLoggedUserId()];
        $scheduledLessonSearchObj = new ScheduledLessonSearch();
    ...
    private function getAgendaForm($testimonialId)
    {
        ...
        $agendafrm->addTextarea(Label::getLabel('LBL_Event_Location', $this->adminLangId), 'events_report_comments_admin_information');
        $agendafrm->addSubmitButton('', 'btn_submit', Label::getLabel('LBL_Reply', $this->adminLangId));
        return $agendafrm;
    }
    ...
    {
        $ReportIssueSection = new SearchBase('tbl_events_report_comments');
        $ReportIssueSection->addCondition('user_id', ' = ', $slesson_id);
        $ReportIssue_categories = $ReportIssueSection->getResultSet();
        $ReportIssueCategoriesList = FatApp::getDb()->fetchAll($ReportIssue_categories);
        $testimonialId = FatUtility::int($testimonialId);
        foreach ($ReportIssueCategoriesList as $key => $value) {
            $agendafrm = $this->getAgendaForm($value['events_report_comments_id']);
        }
        if (0 < $testimonialId) {
        ...
        $this->set('rawData', $ReportIssueCategoriesList);
    ...
    public function agendasetup()
    {
        ...
        $post['events_report_comments_status'] = 1;
        ...
        //  $userData = User::getAttributesById($post['user_id']);
        $db = FatApp::getDb();
        $userSrch = EventUser::getSearchObject(true);
        $userSrch->addMultipleFields(['credential_email']);
        $userSrch->addCondition('credential_user_id', '=', $post['user_id']);
        ...
        $res = EmailHandler::sendSmtpEmail($userData['credential_email'], 'Report Issue', $post['events_report_comments_admin_information'], '', '', $this->siteLangId, '', '');
    ...
    private function searchLessons($post = [], $getCancelledOrder = false, $addLessonDateOrder = true)
    {
        ...
        $srch->addCondition('sldetail_learner_id', '=', EventUserAuthentication::getLoggedUserId());
        $srch->joinTeacherSettings();
    ...
    public function viewBookingCalendar($lDetailId)
    {
        ...
        if (!$lessonRow || $lessonRow['learnerId'] != EventUserAuthentication::getLoggedUserId()) {
            Message::addErrorMessage(Label::getLabel('LBL_INVALID_REQUEST'));
            FatUtility::dieWithError(Message::getHtml());
        }
