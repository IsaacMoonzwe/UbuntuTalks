...
class MeetTheTeamController extends AdminBaseController
{
    ...
    public function search()
    {
        ...
        $srch = MeetTheTeam::getSearchObject($this->adminLangId, false);
        $srch->addMultipleFields(['t.*', 't_l.meet_the_team_title', 't_l.meet_the_team_text']);
        $srch->addOrder('meet_the_team_active', 'desc');
    ...
    public function form($testimonialId)
    {
        ...
        if (0 < $testimonialId) {
            $data = MeetTheTeam::getAttributesById($testimonialId, [
                'meet_the_team_id',
                'meet_the_team_identifier',
                'meet_the_team_active',
                'meet_the_team_user_name',
                'meet_the_team_positions',
                'meet_the_team_description',
                'meet_the_team_positions_listing'
            ]);
            if ($data === false) {
        ...
        $this->set('meet_the_team_id', $testimonialId);
    ...
    public function setup()
    {
        ...
        $testimonialId = $post['meet_the_team_id'];
        unset($post['meet_the_team_id']);
        if ($testimonialId == 0) {
            $post['meet_the_team_added_on'] = date('Y-m-d H:i:s');
        }
        $record = new MeetTheTeam($testimonialId);
        ...
        if ($testimonialId > 0) {
            ...
            foreach ($languages as $langId => $langName) {
                // if (!$row = MeetTheTeam::getAttributesByLangId($langId, $testimonialId)) {
                //     $newTabLangId = $langId;
                //     break;
                // }
            }
    ...
    public function langForm($testimonialId = 0, $lang_id = 0)
    {
        ...
        $langData = MeetTheTeam::getAttributesByLangId($lang_id, $testimonialId);
        if ($langData) {
    ...
    public function langSetup()
    {
        ...
        $data = [
            ...
            'testimoniallang_meet_the_team_id' => $testimonialId,
            'meet_the_team_title' => $post['meet_the_team_title'],
            'meet_the_team_text' => $post['meet_the_team_text']
        ];
        $obj = new MeetTheTeam($testimonialId);
    ...
    public function changeStatus()
    {
        ...
        $data = MeetTheTeam::getAttributesById($testimonialId, ['meet_the_team_id', 'meet_the_team_active']);
        if ($data == false) {
            Message::addErrorMessage($this->str_invalid_request);
            FatUtility::dieWithError(Message::getHtml());
        }
        $status = ($data['meet_the_team_active'] == applicationConstants::ACTIVE) ? applicationConstants::INACTIVE : applicationConstants::ACTIVE;
        $obj = new MeetTheTeam($testimonialId);
    ...
    public function deleteRecord()
    {
        ...
        $testimonialObj = new MeetTheTeam($testimonial_id);
        if (!$testimonialObj->canRecordMarkDelete($testimonial_id)) {
            Message::addErrorMessage($this->str_invalid_request_id);
            FatUtility::dieJsonError(Message::getHtml());
        }
        $testimonialObj->assignValues([MeetTheTeam::tblFld('deleted') => 1]);
    ...
    public function media($testimonialId = 0)
    {
        ...
        $testimonialImages = AttachedFile::getMultipleAttachments(AttachedFile::FILETYPE_MEET_THE_TEAM_IMAGE, $testimonialId, 0, -1);
        $this->set('languages', Language::getAllNames());
        $this->set('testimonialId', $testimonialId);
    ...
    {
        $frm = new Form('frmMeetTheTeamMedia');
        $frm->addButton(Label::getLabel('Lbl_Image', $this->adminLangId), 'testimonial_image', Label::getLabel('LBL_Upload_Image', $this->adminLangId), [
            ...
            'data-file_type' => AttachedFile::FILETYPE_MEET_THE_TEAM_IMAGE,
    ...
    public function uploadTestimonialMedia()
    {
        ...
        $fileHandlerObj->deleteFile($fileHandlerObj::FILETYPE_MEET_THE_TEAM_IMAGE, $testimonialId, 0, 0, $lang_id);
        if (!$res = $fileHandlerObj->saveAttachment($_FILES['file']['tmp_name'], $fileHandlerObj::FILETYPE_MEET_THE_TEAM_IMAGE, $testimonialId, 0, $_FILES['file']['name'], -1, $unique_record = false, $lang_id)) {
    ...
    public function removeTestimonialImage($testimonialId = 0, $lang_id = 0)
    {
        ...
        $fileHandlerObj = new AttachedFile();
        if (!$fileHandlerObj->deleteFile(AttachedFile::FILETYPE_MEET_THE_TEAM_IMAGE, $testimonialId, 0, 0, $lang_id)) {
            Message::addErrorMessage($fileHandlerObj->getError());
            FatUtility::dieJsonError(Message::getHtml());
        }
    ...
    private function getForm($testimonialId = 0)
    {
        $Meet_the_team_positions = [
            ...
            'Senior Board Advisors' => 'Senior Board Advisors', 
            'Directors & Senior Managers' => 'Directors & Senior Managers'
        ];
        $testimonialId = FatUtility::int($testimonialId);
        $frm = new Form('frmMeetTheTeam');
        $frm->addHiddenField('', 'meet_the_team_id', $testimonialId);
        $frm->addHtml('', 'testimonial_image_display_div', '');
        $frm->addRequiredField(Label::getLabel('LBL_meet_the_team_Name', $this->adminLangId), 'meet_the_team_user_name');
        $frm->addRequiredField(Label::getLabel('LBL_meet_the_team_positions', $this->adminLangId), 'meet_the_team_positions');
        $frm->addTextarea(Label::getLabel('LBL_Description', $this->adminLangId), 'meet_the_team_description')->requirements()->setRequired();
        $activeInactiveArr = applicationConstants::getActiveInactiveArr($this->adminLangId);
        $frm->addSelectBox(Label::getLabel('LBL_Meet_The_Team_Position', $langId), 'meet_the_team_positions_listing', $Meet_the_team_positions, -1, [], '');
        $frm->addSelectBox(Label::getLabel('LBL_Status', $this->adminLangId), 'meet_the_team_active', $activeInactiveArr, '', [], '');
    ...
    private function getLangForm($testimonialId = 0, $lang_id = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Testimonial_Title', $this->adminLangId), 'meet_the_team_title');
        $frm->addTextarea(Label::getLabel('LBL_Testimonial_Text', $this->adminLangId), 'meet_the_team_text');
        $frm->addSubmitButton('', 'btn_submit', Label::getLabel('LBL_Save_Changes', $this->adminLangId));
        return $frm;
    }
    ...
    {
        if ($attachment = AttachedFile::getAttachment(AttachedFile::FILETYPE_MEET_THE_TEAM_IMAGE, $testimonialId, 0)) {
            return true;
        }
        return false;
    }
    ...
    public function image($recordId, $langId = 0, $sizeType = '', $afile_id = 0, $displayUniversalImage = true)
    {
        ...
        if ($afile_id > 0) {
            ...
            if (!false == $res && $res['afile_type'] == AttachedFile::FILETYPE_MEET_THE_TEAM_IMAGE) {
                $file_row = $res;
            }
        } else {
