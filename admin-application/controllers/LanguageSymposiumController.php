...
class LanguageSymposiumController extends AdminBaseController
{
    ...
    {
        $srch = LanguageSymposium::getSearchObject($this->adminLangId, false);
        $srch->addOrder('language_symposium_active', 'DESC');
    ...
    public function form($epage_id = 0)
    {
        ...
        if (0 < $epage_id) {
            $data = LanguageSymposium::getAttributesById($epage_id, array('epage_id', 'language_symposium_identifier', 'language_symposium_active'));
            if ($data === false) {
    ...
    public function setup()
    {
        ...
        $extraPage = new LanguageSymposium($epage_id);
        ...
        foreach ($languages as $langId => $langName) {
            if (!$row = LanguageSymposium::getAttributesByLangId($langId, $epage_id)) {
                $newTabLangId = $langId;
                break;
            }
        }
    ...
    public function langForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $epageData = LanguageSymposium::getAttributesById($epage_id);
        $blockLangFrm = $this->getLangForm($epage_id, $lang_id);
        $langData = LanguageSymposium::getAttributesByLangId($lang_id, $epage_id);
        if ($langData) {
    ...
    public function langSetup()
    {
        ...
        $data = array(
            ...
            'language_symposium_label' => $post['language_symposium_label'],
            'language_symposium_content' => $post['language_symposium_content'],
        );
        $epageObj = new LanguageSymposium($epage_id);
    ...
    public function changeStatus()
    {
        ...
        $contentBlockData = LanguageSymposium::getAttributesById($epageId, array('language_symposium_active'));
        if (!$contentBlockData) {
            Message::addErrorMessage($this->str_invalid_request);
            FatUtility::dieWithError(Message::getHtml());
        }
        $status = ($contentBlockData['language_symposium_active'] == applicationConstants::ACTIVE) ? applicationConstants::INACTIVE : applicationConstants::ACTIVE;
    ...
    private function updateEPageStatus($epageId, $status)
    {
        ...
        $EPageObj = new LanguageSymposium($epageId);
    ...
    private function getLangForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Title', $this->adminLangId), 'language_symposium_label');
        $frm->addHtmlEditor(Label::getLabel('LBL_Page_Content', $this->adminLangId), 'language_symposium_content');
