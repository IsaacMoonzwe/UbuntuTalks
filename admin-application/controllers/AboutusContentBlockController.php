...
class AboutusContentBlockController extends AdminBaseController
{
    ...
    {
        $srch = AboutusContentBlock::getSearchObject($this->adminLangId, false);
        $srch->addOrder('aboutus_content_block_active', 'DESC');
    ...
    public function form($epage_id = 0)
    {
        ...
        if (0 < $epage_id) {
            $data = AboutusContentBlock::getAttributesById($epage_id, array('epage_id', 'aboutus_content_block_identifier', 'aboutus_content_block_active'));
            if ($data === false) {
    ...
    public function setup()
    {
        ...
        $extraPage = new AboutusContentBlock($epage_id);
        ...
        foreach ($languages as $langId => $langName) {
            if (!$row = AboutusContentBlock::getAttributesByLangId($langId, $epage_id)) {
                $newTabLangId = $langId;
                break;
            }
        }
    ...
    public function langForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $epageData = AboutusContentBlock::getAttributesById($epage_id);
        $blockLangFrm = $this->getLangForm($epage_id, $lang_id);
        $langData = AboutusContentBlock::getAttributesByLangId($lang_id, $epage_id);
        if ($langData) {
    ...
    public function langSetup()
    {
        ...
        $data = array(
            ...
            'aboutus_content_block_label' => $post['aboutus_content_block_label'],
            'aboutus_content_block_content' => $post['aboutus_content_block_content'],
        );
        $epageObj = new AboutusContentBlock($epage_id);
    ...
    public function changeStatus()
    {
        ...
        $contentBlockData = AboutusContentBlock::getAttributesById($epageId, array('aboutus_content_block_active'));
        if (!$contentBlockData) {
            Message::addErrorMessage($this->str_invalid_request);
            FatUtility::dieWithError(Message::getHtml());
        }
        $status = ($contentBlockData['aboutus_content_block_active'] == applicationConstants::ACTIVE) ? applicationConstants::INACTIVE : applicationConstants::ACTIVE;
    ...
    private function getForm($epage_id = 0, $langId = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Identifier', $this->adminLangId), 'aboutus_content_block_identifier');

        $activeInactiveArr = applicationConstants::getActiveInactiveArr($langId);
        $frm->addSelectBox(Label::getLabel('LBL_Status', $this->adminLangId), 'aboutus_content_block_active', $activeInactiveArr, '', array(), '');
    ...
    private function getLangForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Title', $this->adminLangId), 'aboutus_content_block_label');
        $frm->addHtmlEditor(Label::getLabel('LBL_Page_Content', $this->adminLangId), 'aboutus_content_block_content');
