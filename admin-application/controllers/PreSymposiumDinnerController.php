<?php
class PreSymposiumDinnerController extends AdminBaseController
{
    ...
    {
        $srch = PreSymposiumDinner::getSearchObject($this->adminLangId, false);
        $srch->addMultipleFields(['t.*', 't_l.pre_symposium_dinner_title', 't_l.pre_symposium_dinner_text']);
        $srch->addOrder('pre_symposium_dinner_active', 'desc');
        $records = FatApp::getDb()->fetchAll($srch->getResultSet());
        foreach ($records as $key => $value) {
            $testimonialImages = AttachedFile::getMultipleAttachments(AttachedFile::FILETYPE_EVENT_PLAN_IMAGE, $value['pre_symposium_dinner_id'], 0, -1);
    ...
    public function form($testimonialId)
    {
        ...
        if (0 < $testimonialId) {
            $data = PreSymposiumDinner::getAttributesById($testimonialId);
            if ($data === false) {
        ...
        $this->set('pre_symposium_dinner_id', $testimonialId);
    ...
    public function setup()
    {
        ...
        $user_food_department  = implode(',', $newPost['pre_symposium_dinner_plan_combo_events']);
        if (isset($newPost['pre_symposium_dinner_plan_combo_events']) && !empty($newPost['pre_symposium_dinner_plan_combo_events']) && $user_food_department[0] != '') {
            $user_food_department  = implode(',', $newPost['pre_symposium_dinner_plan_combo_events']);
            $post['pre_symposium_dinner_plan_combo_events'] = $user_food_department;
        } else {
            $post['pre_symposium_dinner_plan_combo_events'] = "";
        }
        $testimonialId = $post['pre_symposium_dinner_id'];
        unset($post['pre_symposium_dinner_id']);
        if ($testimonialId == 0) {
            $post['pre_symposium_dinner_added_on'] = date('Y-m-d H:i:s');
        }
        $record = new PreSymposiumDinner($testimonialId);
        ...
        if ($testimonialId > 0) {
            ...
            foreach ($languages as $langId => $langName) {
                // if (!$row = PreSymposiumDinner::getAttributesByLangId($langId, $testimonialId)) {
                //     $newTabLangId = $langId;
                //     break;
                // }
            }
    ...
    public function langForm($testimonialId = 0, $lang_id = 0)
    {
        ...
        $langData = PreSymposiumDinner::getAttributesByLangId($lang_id, $testimonialId);
        if ($langData) {
    ...
    public function langSetup()
    {
        ...
        $data = [
            ...
            'pre_symposium_dinner_title' => $post['pre_symposium_dinner_title'],
            'pre_symposium_dinner_text' => $post['pre_symposium_dinner_text']
        ];
        $obj = new PreSymposiumDinner($testimonialId);
    ...
    public function updateOrder()
    {
        ...
        $post = FatApp::getPostedData();
        if (!empty($post)) {
            $pMethodObj = new PreSymposiumDinner();
    ...
    public function changeStatus()
    {
        ...
        $data = PreSymposiumDinner::getAttributesById($testimonialId, ['pre_symposium_dinner_id', 'pre_symposium_dinner_active']);
        if ($data == false) {
            Message::addErrorMessage($this->str_invalid_request);
            FatUtility::dieWithError(Message::getHtml());
        }
        $status = ($data['pre_symposium_dinner_active'] == applicationConstants::ACTIVE) ? applicationConstants::INACTIVE : applicationConstants::ACTIVE;
        $obj = new PreSymposiumDinner($testimonialId);
    ...
    public function deleteRecord()
    {
        ...
        $testimonialObj = new PreSymposiumDinner($testimonial_id);
        if (!$testimonialObj->canRecordMarkDelete($testimonial_id)) {
            Message::addErrorMessage($this->str_invalid_request_id);
            FatUtility::dieJsonError(Message::getHtml());
        }
        $testimonialObj->assignValues([PreSymposiumDinner::tblFld('deleted') => 1]);
    ...
    private function getForm($testimonialId = 0)
    {
        ...
        $frm->addHiddenField('', 'pre_symposium_dinner_id', $testimonialId);
        $frm->addHtml('', 'testimonial_image_display_div', '');
        $frm->addRequiredField(Label::getLabel('LBL_pre_symposium_dinner_Plan_Title', $this->adminLangId), 'pre_symposium_dinner_plan_title');
        $frm->addRequiredField(Label::getLabel('LBL_pre_symposium_dinner_Starting_Days_(Date)'), 'pre_symposium_dinner_starting_date', '', ['id' => 'pre_symposium_dinner_starting_date', 'autocomplete' => 'off']);
        ...
        // $select_plan_listing = new SearchBase('tbl_pre_symposium_dinner');
        // $select_plan_listing->addCondition('pre_symposium_dinner_deleted', '=', 0);
        // $select_plan_listing->addCondition('pre_symposium_dinner_active', '=', 1);
        // if ($testimonialId > 0) {
        //     $select_plan_listing->addCondition('pre_symposium_dinner_id', '=', $testimonialId);
        // }
        // $select_events_listings = $select_plan_listing->getResultSet();
        // $SelectEventListingsList = FatApp::getDb()->fetch($select_events_listings);
        // $srch_plan_listing = new SearchBase('tbl_pre_symposium_dinner');
        // $srch_plan_listing->addCondition('pre_symposium_dinner_deleted', '=', 0);
        // $srch_plan_listing->addCondition('pre_symposium_dinner_active', '=', 1);
        // if ($testimonialId > 0) {
        //     $srch_plan_listing->addCondition('pre_symposium_dinner_id', '!=', $testimonialId);
        // }
        ...
        //     $week =  $value['pre_symposium_dinner_plan_title'];
        //     $id =  $value['pre_symposium_dinner_id'];
        //     // $speekLangField = $frm->addCheckBox($week, 'pre_symposium_dinner_plan_combo_events[' . $key . ']', $id, ['class' => 'diet-boxes'], false, 0);
        //     $speekLangField = $frm->addCheckBox($week, 'pre_symposium_dinner_plan_combo_events[]', $id, ['class' => 'diet-boxes'], false, 0);
        //     if ($testimonialId > 0) {
        //         $select = explode(",", $SelectEventListingsList['pre_symposium_dinner_plan_combo_events']);
        ...
        //     $speekLangField->value = $id;
        // }
        $frm->addRequiredField(Label::getLabel('LBL_Total_Available_Tickets', $this->adminLangId), 'pre_symposium_dinner_avilable_tickets');
        $frm->addSelectBox(Label::getLabel('LBL_Status', $this->adminLangId), 'pre_symposium_dinner_active', $activeInactiveArr, '', [], '');
    ...
    private function getLangForm($testimonialId = 0, $lang_id = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Testimonial_Title', $this->adminLangId), 'pre_symposium_dinner_title');
        $frm->addTextarea(Label::getLabel('LBL_Testimonial_Text', $this->adminLangId), 'pre_symposium_dinner_text');
        $frm->addSubmitButton('', 'btn_submit', Label::getLabel('LBL_Save_Changes', $this->adminLangId));
        return $frm;
    }
