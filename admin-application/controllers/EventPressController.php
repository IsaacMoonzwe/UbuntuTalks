...
class EventPressController extends AdminBaseController
{
    ...
    {
        $srch = EventPress::getSearchObject($this->adminLangId, false);
        $srch->addOrder('event_press_active', 'DESC');
    ...
    public function form($epage_id = 0)
    {
        ...
        if (0 < $epage_id) {
            $data = EventPress::getAttributesById($epage_id, array('epage_id', 'event_press_identifier', 'event_press_active'));
            if ($data === false) {
    ...
    public function setup()
    {
        ...
        $extraPage = new EventPress($epage_id);
        ...
        foreach ($languages as $langId => $langName) {
            if (!$row = EventPress::getAttributesByLangId($langId, $epage_id)) {
                $newTabLangId = $langId;
                break;
            }
        }
    ...
    public function langForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $epageData = EventPress::getAttributesById($epage_id);
        $blockLangFrm = $this->getLangForm($epage_id, $lang_id);
        $langData = EventPress::getAttributesByLangId($lang_id, $epage_id);
        if ($langData) {
    ...
    public function langSetup()
    {
        ...
        $data = array(
            ...
            'event_press_label' => $post['event_press_label'],
            'event_press_content' => $post['event_press_content'],
        );
        $epageObj = new EventPress($epage_id);
    ...
    public function changeStatus()
    {
        ...
        $contentBlockData = EventPress::getAttributesById($epageId, array('event_press_active'));
        if (!$contentBlockData) {
            Message::addErrorMessage($this->str_invalid_request);
            FatUtility::dieWithError(Message::getHtml());
        }
        $status = ($contentBlockData['event_press_active'] == applicationConstants::ACTIVE) ? applicationConstants::INACTIVE : applicationConstants::ACTIVE;
    ...
    private function updateEPageStatus($epageId, $status)
    {
        ...
        $EPageObj = new EventPress($epageId);
    ...
    private function getForm($epage_id = 0, $langId = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Identifier', $this->adminLangId), 'event_press_identifier');

        $activeInactiveArr = applicationConstants::getActiveInactiveArr($langId);
        $frm->addSelectBox(Label::getLabel('LBL_Status', $this->adminLangId), 'event_press_active', $activeInactiveArr, '', array(), '');
    ...
    private function getLangForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Title', $this->adminLangId), 'event_press_label');
        $frm->addHtmlEditor(Label::getLabel('LBL_Page_Content', $this->adminLangId), 'event_press_content');
