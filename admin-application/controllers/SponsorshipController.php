...
class SponsorshipController extends AdminBaseController
{
    ...
    {
        $srch = Sponsorship::getSearchObject($this->adminLangId, false);
        $srch->addMultipleFields(['t.*', 't_l.sponsorship_title', 't_l.sponsorship_text']);
        $srch->addOrder('sponsorship_active', 'desc');
    ...
    public function form($testimonialId)
    {
        ...
        if (0 < $testimonialId) {
            $data = Sponsorship::getAttributesById($testimonialId, [
                        'sponsorship_id',
                        'sponsorship_identifier',
                        'sponsorship_active',
                        'sponsorship_user_name',
                        'sponsorship_positions',
                        'sponsorship_description',
                        'sponsorshipcategories_url'
            ]);
            if ($data === false) {
        ...
        $this->set('sponsorship_id', $testimonialId);
    ...
    public function setup()
    {
        ...
        $testimonialId = $post['sponsorship_id'];
        unset($post['sponsorship_id']);
        if ($testimonialId == 0) {
            $post['sponsorship_added_on'] = date('Y-m-d H:i:s');
        }
        $record = new Sponsorship($testimonialId);
    ...
    public function langForm($testimonialId = 0, $lang_id = 0)
    {
        ...
        $langData = Sponsorship::getAttributesByLangId($lang_id, $testimonialId);
        if ($langData) {
    ...
    public function langSetup()
    {
        ...
        $data = [
            ...
            'testimoniallang_sponsorship_id' => $testimonialId,
            'sponsorship_title' => $post['sponsorship_title'],
            'sponsorship_text' => $post['sponsorship_text']
        ];
        $obj = new Sponsorship($testimonialId);
    ...
    public function changeStatus()
    {
        ...
        $data = Sponsorship::getAttributesById($testimonialId, ['sponsorship_id', 'sponsorship_active']);
        if ($data == false) {
    ...
    public function deleteRecord()
    {
        ...
        $testimonialObj = new Sponsorship($testimonial_id);
        if (!$testimonialObj->canRecordMarkDelete($testimonial_id)) {
            Message::addErrorMessage($this->str_invalid_request_id);
            FatUtility::dieJsonError(Message::getHtml());
        }
        $testimonialObj->assignValues([Sponsorship::tblFld('deleted') => 1]);
    ...
    public function media($testimonialId = 0)
    {
        ...
        $testimonialImages = AttachedFile::getMultipleAttachments(AttachedFile::FILETYPE_SPONSORSHIP_IMAGE, $testimonialId, 0, -1);
        $this->set('languages', Language::getAllNames());
        $this->set('testimonialId', $testimonialId);
    ...
    public function getMediaForm($testimonialId)
    {
        ...
        $frm->addButton(Label::getLabel('Lbl_Image', $this->adminLangId), 'testimonial_image', Label::getLabel('LBL_Upload_Image', $this->adminLangId), [
            ...
            'data-file_type' => AttachedFile::FILETYPE_SPONSORSHIP_IMAGE,
    ...
    public function removeTestimonialImage($testimonialId = 0, $lang_id = 0)
    {
        ...
        $fileHandlerObj = new AttachedFile();
        if (!$fileHandlerObj->deleteFile(AttachedFile::FILETYPE_SPONSORSHIP_IMAGE, $testimonialId, 0, 0, $lang_id)) {
            Message::addErrorMessage($fileHandlerObj->getError());
            FatUtility::dieJsonError(Message::getHtml());
        }
    ...
    private function getForm($testimonialId = 0)
    {
        ...
        $frm->addHiddenField('', 'sponsorship_id', $testimonialId);
        $frm->addHtml('', 'testimonial_image_display_div', '');
        $frm->addSelectBox(Label::getLabel('LBL_Sponsorship_Type', $langId), 'sponsorhip_type', $sponsorship_type, -1, [], '');
        $frm->addRequiredField(Label::getLabel('LBL_sponsorship_Name', $this->adminLangId), 'sponsorship_user_name');
        $frm->addRequiredField(Label::getLabel('LBL_sponsorship_Positions', $this->adminLangId), 'sponsorship_positions');
        $frm->addRequiredField(Label::getLabel('LBL_URL', $this->adminLangId), 'sponsorshipcategories_url');
        $frm->addTextarea(Label::getLabel('LBL_Description', $this->adminLangId), 'sponsorship_description')->requirements()->setRequired();
        $activeInactiveArr = applicationConstants::getActiveInactiveArr($this->adminLangId);
        $frm->addSelectBox(Label::getLabel('LBL_Status', $this->adminLangId), 'sponsorship_active', $activeInactiveArr, '', [], '');
    ...
    private function getLangForm($testimonialId = 0, $lang_id = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Testimonial_Title', $this->adminLangId), 'sponsorship_title');
        $frm->addTextarea(Label::getLabel('LBL_Testimonial_Text', $this->adminLangId), 'sponsorship_text');
        $frm->addSubmitButton('', 'btn_submit', Label::getLabel('LBL_Save_Changes', $this->adminLangId));
        return $frm;
    }
    ...
    {
        if ($attachment = AttachedFile::getAttachment(AttachedFile::FILETYPE_SPONSORSHIP_IMAGE, $testimonialId, 0)) {
            return true;
        }
        return false;
    }
