...
(function() {
	...
	searchTestimonial = function(frm){
		...
		fcom.ajax(fcom.makeUrl('VirtualSession','search'),data,function(res){
			$(dv).html(res);			
		});
	};
	...
	testimonialForm = function(id) {
		...
		//$.facebox(function() {
			fcom.ajax(fcom.makeUrl('VirtualSession', 'form', [id]), '', function(t) {
				//$.facebox(t,'faceboxWidth');
				fcom.updateFaceboxContent(t);
				jQuery('#virtual_session_start_time').each(function () {
					$(this).datetimepicker({
						format: 'Y-m-d H:i',
	...
	editTestimonialForm = function(testimonialId){
		...
		//$.facebox(function() {
			fcom.ajax(fcom.makeUrl('VirtualSession', 'form', [testimonialId]), '', function(t) {
				//$.facebox(t,'faceboxWidth');
				fcom.updateFaceboxContent(t);
				jQuery('#virtual_session_start_time').each(function () {
					$(this).datetimepicker({
	...
	setupTestimonial = function (frm){
		...
		fcom.updateWithAjax(fcom.makeUrl('VirtualSession', 'setup'), data, function(t) {			
			//$.mbsmessage.close();
			reloadList();
			if (t.langId>0) {
	...
	editTestimonialLangForm = function(testimonialId,langId){
		fcom.displayProcessing();
	//	$.facebox(function() {
			fcom.ajax(fcom.makeUrl('VirtualSession', 'langForm', [testimonialId,langId]), '', function(t) {
				//$.facebox(t,'faceboxWidth');
				fcom.updateFaceboxContent(t);
			});
		//});
	...
	setupLangTestimonial = function (frm){
		if (!$(frm).validate()) return;		
		var data = fcom.frmData(frm);
		fcom.updateWithAjax(fcom.makeUrl('VirtualSession', 'langSetup'), data, function(t) {
			reloadList();			
			if (t.langId>0) {
				editTestimonialLangForm(t.testimonialId, t.langId);
				return ;
	...
	deleteRecord = function(id){
		...
		fcom.updateWithAjax(fcom.makeUrl('VirtualSession', 'deleteRecord'),data,function(res){		
			reloadList();
		});
	};
	...
	activeStatus = function(obj){
		...
		fcom.ajax(fcom.makeUrl('VirtualSession','changeStatus'),data,function(res){
	...
	testimonialMediaForm = function(testimonialId){
		//$.facebox(function() {
			fcom.displayProcessing();
			fcom.ajax(fcom.makeUrl('VirtualSession', 'media', [testimonialId]), '', function(t) {
				//$.facebox(t);
				fcom.updateFaceboxContent(t);
			});
		//});
	...
	removeTestimonialImage = function( testimonialId, langId ){
		...
		fcom.updateWithAjax(fcom.makeUrl('VirtualSession', 'removeTestimonialImage',[testimonialId, langId]), '', function(t) {
...
$(document).on('click','.uploadFile-Js',function(){
	...
	timer = setInterval(function() {
		if ($('#form-upload input[name=\'file\']').val() != '') {
			...
			$val = $(node).val();			
			$.ajax({
				url: fcom.makeUrl('VirtualSession', 'uploadTestimonialMedia'),
				type: 'post',
				dataType: 'json',
