...
if ($cpage_layout == ContentPage::CONTENT_PAGE_LAYOUT1_TYPE) {
    ...
    $preferredDimensionsStr = '<small class="text--small"> ' . Label::getLabel('LBL_This_will_be_displayed_on_your_cms_Page', $adminLangId) . '</small>';
    $htmlAfterField = $preferredDimensionsStr;
    if (!empty($bgImages)) {
        $htmlAfterField .= '<ul class="image-listing grids--onethird">';
        foreach ($bgImages as $bgImage) {
            $htmlAfterField .= '<li>' . $bannerTypeArr[$bgImage['afile_lang_id']] . '<div class="uploaded--image"><img src="' . CommonHelper::generateFullUrl('ContentPages', 'cpageBackgroundImage', array($bgImage['afile_record_id'], $bgImage['afile_lang_id'], 'THUMB')) . '?' . time() . '"> <a href="javascript:void(0);" onClick="removeBgImage(' . $bgImage['afile_record_id'] . ',' . $bgImage['afile_lang_id'] . ',' . $cpage_layout . ')" class="remove--img"><i class="ion-close-round"></i></a></div>';
        }
...
<section class="section">
    <div class="sectionhead">
        <h4><?php echo Label::getLabel('LBL_Content_Pages_Setup', $adminLangId); ?></h4>
    </div>
    <div class="sectionbody space">
        <div class="row">
            <div class="col-sm-12">
                <h1><?php //echo Label::getLabel('LBL_Content_Pages_Setup',$adminLangId);       ?></h1>
                <div class="tabs_nav_container responsive flat">
                    <ul class="tabs_nav">
                        <li><a href="javascript:void(0);" onclick="addForm(<?php echo $cpage_id ?>);"><?php echo Label::getLabel('LBL_General', $adminLangId); ?></a></li>
                        <?php
                        if ($cpage_id > 0) {
