(function () {
    ...
    viewDetail = function (lessonId) {
        $.facebox(function () {
            fcom.ajax(fcom.makeUrl('PreSymposiumDinnerInformation', 'viewDetail', [lessonId]), '', function (t) {
    ...
    reportDetail = function(lessonId){
		$.facebox(function() {
			fcom.ajax(fcom.makeUrl('PreSymposiumDinnerInformation', 'reportDetail', [lessonId]), '', function(t) {
			$.facebox(t,'faceboxWidth');
        ...
        agendaSetupTestimonial = function (frm){
            ...
            fcom.updateWithAjax(fcom.makeUrl('PreSymposiumDinnerInformation', 'agendasetup'), data, function(t) {		
    ...
    updateAssignClassStatus = function(obj, id, value, oldValue){
		...
		fcom.ajax(fcom.makeUrl('PreSymposiumDinnerInformation','updateAssignClass'),{"sldetail_id":id, "teachers_id" : value},function(json){
			res = $.parseJSON(json);
    ...
    ChangeStatus = function(value){
		...
		fcom.ajax(fcom.makeUrl('PreSymposiumDinnerInformation','changeStatus',[value]),"",function(json){
			res = $.parseJSON(json);
            $(div).html(fcom.getLoader());
			if(res.status == "1"){
				  $.mbsmessage( res.msg,true, 'alert alert--success');
    ...
    updateScheduleStatus = function (obj, id, value, oldValue) {
        ...
        fcom.ajax(fcom.makeUrl('PreSymposiumDinnerInformation', 'updateStatusSetup'), {"sldetail_id": id, "slesson_status": value}, function (json) {
            res = $.parseJSON(json);
            if (res.status == "1") {
                $.mbsmessage(res.msg, true, 'alert alert--success');
            } else {
    ...
    searchPuchasedLessons = function (form) {
        ...
        $(div).html(fcom.getLoader());
        fcom.ajax(fcom.makeUrl('PreSymposiumDinnerInformation', 'purchasedLessonsSearch'), data, function (res) {
            $(div).html(res);
        });
    };
