...
(function() {
	...
	searchTestimonial = function(frm){
		...
		fcom.ajax(fcom.makeUrl('EventsSponsorshipCategories','search'),data,function(res){
			$(dv).html(res);			
		});
	};
	...
	editAgendaFormNew = function(testimonialId){
		...
		//$.facebox(function() {
			fcom.ajax(fcom.makeUrl('EventsSponsorshipCategories', 'form', [testimonialId]), '', function(t) {
				//$.facebox(t,'faceboxWidth');
				fcom.updateFaceboxContent(t);
				jQuery('#events_sponsorship_categories_starting_date,#events_sponsorship_categories_ending_date').each(function () {
					$(this).datetimepicker({
						format: 'Y-m-d H:i',
	...
	testimonialForm = function(id) {
		fcom.displayProcessing();
		//$.facebox(function() {
			fcom.ajax(fcom.makeUrl('EventsSponsorshipCategories', 'form', [id]), '', function(t) {
				//$.facebox(t,'faceboxWidth');
				fcom.updateFaceboxContent(t);
				jQuery('#events_sponsorship_categories_starting_date,#events_sponsorship_categories_ending_date').each(function () {
					$(this).datetimepicker({
	...
	setupTestimonial = function (frm){
		...
		fcom.updateWithAjax(fcom.makeUrl('EventsSponsorshipCategories', 'setup'), data, function(t) {			
			//$.mbsmessage.close();
			reloadList();
			if (t.langId>0) {
	...
	editTestimonialLangForm = function(testimonialId,langId){
		fcom.displayProcessing();
	//	$.facebox(function() {
			fcom.ajax(fcom.makeUrl('EventsSponsorshipCategories', 'langForm', [testimonialId,langId]), '', function(t) {
				//$.facebox(t,'faceboxWidth');
				fcom.updateFaceboxContent(t);
			});
		//});
	...
	setupLangTestimonial = function (frm){
		if (!$(frm).validate()) return;		
		var data = fcom.frmData(frm);
		fcom.updateWithAjax(fcom.makeUrl('EventsSponsorshipCategories', 'langSetup'), data, function(t) {
			reloadList();			
			if (t.langId>0) {
				editTestimonialLangForm(t.testimonialId, t.langId);
				return ;
	...
	deleteRecord = function(id){
		...
		fcom.updateWithAjax(fcom.makeUrl('EventsSponsorshipCategories', 'deleteRecord'),data,function(res){		
			reloadList();
		});
	};
	...
	activeStatus = function(obj){
		...
		fcom.ajax(fcom.makeUrl('EventsSponsorshipCategories','changeStatus'),data,function(res){
	...
	testimonialMediaForm = function(testimonialId){
		//$.facebox(function() {
			fcom.displayProcessing();
			fcom.ajax(fcom.makeUrl('EventsSponsorshipCategories', 'media', [testimonialId]), '', function(t) {
				//$.facebox(t);
				fcom.updateFaceboxContent(t);
			});
		//});
	...
	removeTestimonialImage = function( testimonialId, langId ){
		...
		fcom.updateWithAjax(fcom.makeUrl('EventsSponsorshipCategories', 'removeTestimonialImage',[testimonialId, langId]), '', function(t) {
...
$(document).on('click','.uploadFile-Js',function(){
	...
	timer = setInterval(function() {
		if ($('#form-upload input[name=\'file\']').val() != '') {
			...
			$val = $(node).val();			
			$.ajax({
				url: fcom.makeUrl('EventsSponsorshipCategories', 'uploadTestimonialMedia'),
				type: 'post',
				dataType: 'json',
