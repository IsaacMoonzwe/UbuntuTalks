...
(function() {
	...
	viewDetail = function(lessonId){
		$.facebox(function() {
			fcom.ajax(fcom.makeUrl('TransactionHistory', 'viewDetail', [lessonId]), '', function(t) {
	...
	reportDetail = function(lessonId){
		$.facebox(function() {
			fcom.ajax(fcom.makeUrl('TransactionHistory', 'reportDetail', [lessonId]), '', function(t) {
			$.facebox(t,'faceboxWidth');
			});
		});
	};
	...
	serachScheduledLesson = function(page){
		...
		fcom.ajax(fcom.makeUrl('TransactionHistory','purchasedLessonsSearch'), data,function(res){
			$(dv).html(res);
		});
	};
	...
	updatePayment = function (frm){
		...
		fcom.updateWithAjax(fcom.makeUrl('TransactionHistory', 'updatePayment'), data, function(t) {
	...
	updateAssignClassStatus = function(obj, id, value, oldValue){
		...
		fcom.ajax(fcom.makeUrl('TransactionHistory','updateAssignClass'),{"sldetail_id":id, "slesson_status" : value},function(json){
			res = $.parseJSON(json);
			if(res.status == "1"){
	...
	updateScheduleStatus = function(obj, id, value, oldValue){
		...
		fcom.ajax(fcom.makeUrl('TransactionHistory','updateStatusSetup'),{"sldetail_id":id, "slesson_status" : value},function(json){
			res = $.parseJSON(json);
			if(res.status == "1"){
				  $.mbsmessage( res.msg,true, 'alert alert--success');
			}else{
