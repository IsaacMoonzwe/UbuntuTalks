...
(function() {
	...
	searchBlocks = function(form){
		...
		fcom.ajax(fcom.makeUrl('LanguageSymposium','search'),data,function(res){
			$(dv).html(res);
		});
	};
	...
	addBlockForm = function(id) {
		...
		var frm = document.frmBlockSearch;			
			fcom.ajax(fcom.makeUrl('LanguageSymposium', 'form', [id]), '', function(t) {
	...
	setupBlock = function(frm) {
		...
		fcom.updateWithAjax(fcom.makeUrl('LanguageSymposium', 'setup'), data, function(t) {
			if ( t.langId > 0 ) {
	...
	addBlockLangForm = function(epageId, langId){
		...
		fcom.resetEditorInstance();
			fcom.ajax(fcom.makeUrl('LanguageSymposium', 'langForm', [epageId, langId]), '', function(t) {
				...
				$(frm).submit(function(e) {
					...
					var data = fcom.frmData(frm);
					fcom.updateWithAjax(fcom.makeUrl('LanguageSymposium', 'langSetup'), data, function(t) {
						fcom.resetEditorInstance();	
						reloadList();				
						if (t.langId>0) {
							addBlockLangForm(t.epageId, t.langId);
	...
	activeStatus = function(obj){
		...
		fcom.ajax(fcom.makeUrl('LanguageSymposium','changeStatus'),data,function(res){
	...
	removeBgImage = function( epage_id, langId, file_type ){
		...
		fcom.updateWithAjax( fcom.makeUrl('LanguageSymposium', 'removeBgImage',[epage_id, langId, file_type]), '', function(t) {
