...
(function() {
	...
	searchFaq = function(form){		
		...
		fcom.ajax(fcom.makeUrl('FreeDays','search'),data,function(res){
			$(dv).html(res);			
		});
	};
	...
	FaqForm = function(id) {
		...
		//$.facebox(function() {
			fcom.ajax(fcom.makeUrl('FreeDays', 'form', [id]), '', function(t) {
	...
	editFaqForm = function(faqId){
		fcom.displayProcessing();
		//$.facebox(function() {
			fcom.ajax(fcom.makeUrl('FreeDays', 'form', [faqId]), '', function(t) {
				//$.facebox(t,'faceboxWidth');
				fcom.updateFaceboxContent(t);
			});
		//});
	...
	setupFaq = function (frm){
		...
		fcom.updateWithAjax(fcom.makeUrl('FreeDays', 'setup'), data, function(t) {			
			//$.mbsmessage.close();
			reloadList();
			if (t.langId>0) {
	...
	editFaqLangForm = function(faqId,langId){
		fcom.displayProcessing();
	//	$.facebox(function() {
			fcom.ajax(fcom.makeUrl('FreeDays', 'langForm', [faqId,langId]), '', function(t) {
				//$.facebox(t,'faceboxWidth');
				fcom.updateFaceboxContent(t);
			});
		//});
	...
	setupLangFaq = function (frm){
		if (!$(frm).validate()) return;		
		var data = fcom.frmData(frm);
		fcom.updateWithAjax(fcom.makeUrl('FreeDays', 'langSetup'), data, function(t) {
			reloadList();			
			if (t.langId>0) {
				editFaqLangForm(t.faqId, t.langId);
				return ;
	...
	deleteRecord = function(id){
		...
		data='faqId='+id;
		fcom.updateWithAjax(fcom.makeUrl('FreeDays', 'deleteRecord'),data,function(res){		
			reloadList();
		});
	};
	...
	activeStatus = function(obj){
		...
		fcom.ajax(fcom.makeUrl('FreeDays','changeStatus'),data,function(res){
