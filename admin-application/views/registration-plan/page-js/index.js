...
(function() {
	...
	searchTestimonial = function(frm){
		...
		fcom.ajax(fcom.makeUrl('RegistrationPlan','search'),data,function(res){
			$(dv).html(res);			
		});
	};
	...
	setupTestimonial = function (frm){
		...
		fcom.updateWithAjax(fcom.makeUrl('RegistrationPlan', 'setup'), data, function(t) {			
			//$.mbsmessage.close();
			reloadList();
			if (t.langId>0) {
	...
	editTestimonialLangForm = function(testimonialId,langId){
		fcom.displayProcessing();
	//	$.facebox(function() {
			fcom.ajax(fcom.makeUrl('RegistrationPlan', 'langForm', [testimonialId,langId]), '', function(t) {
				//$.facebox(t,'faceboxWidth');
				fcom.updateFaceboxContent(t);
			});
		//});
	...
	setupLangTestimonial = function (frm){
		if (!$(frm).validate()) return;		
		var data = fcom.frmData(frm);
		fcom.updateWithAjax(fcom.makeUrl('RegistrationPlan', 'langSetup'), data, function(t) {
			reloadList();			
			if (t.langId>0) {
				editTestimonialLangForm(t.testimonialId, t.langId);
				return ;
	...
	deleteRecord = function(id){
		...
		fcom.updateWithAjax(fcom.makeUrl('RegistrationPlan', 'deleteRecord'),data,function(res){		
			reloadList();
		});
	};
	...
	activeStatus = function(obj){
		...
		fcom.ajax(fcom.makeUrl('RegistrationPlan','changeStatus'),data,function(res){
	...
	testimonialMediaForm = function(testimonialId){
		//$.facebox(function() {
			fcom.displayProcessing();
			fcom.ajax(fcom.makeUrl('RegistrationPlan', 'media', [testimonialId]), '', function(t) {
				//$.facebox(t);
				fcom.updateFaceboxContent(t);
			});
		//});
	...
	removeTestimonialImage = function( testimonialId, langId ){
		...
		fcom.updateWithAjax(fcom.makeUrl('RegistrationPlan', 'removeTestimonialImage',[testimonialId, langId]), '', function(t) {
...
$(document).on('click','.uploadFile-Js',function(){
	...
	timer = setInterval(function() {
		if ($('#form-upload input[name=\'file\']').val() != '') {
			...
			$val = $(node).val();			
			$.ajax({
				url: fcom.makeUrl('RegistrationPlan', 'uploadTestimonialMedia'),
				type: 'post',
				dataType: 'json',
