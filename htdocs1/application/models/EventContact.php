...
class EventContact extends MyAppModel
{

    const DB_TBL = 'tbl_event_contact';
    const DB_TBL_PREFIX = 'epage_';
    const DB_TBL_LANG = 'tbl_event_contact_lang';
    const DB_TBL_LANG_PREFIX = 'epagelang_';
    const BLOCK_CONTACT_INFORMATION = 26;
    const BLOCK_PRIVACY_INFORMATION = 27;
    ...
    public static function getSearchObject($langId = 0, $isActive = true)
    {
        ...
        if ($isActive) {
            $srch->addCondition('event_contact_active', '=', applicationConstants::ACTIVE);
        }
        return $srch;
    }
    ...
    public static function getBlockContent(int $pageType, int $langId = 0): string
    {
        ...
        $srch->addCondition('ep.event_contact_type', '=', $pageType);
        $srch->addMultipleFields(array('epage_id', 'IFNULL(event_contact_label, event_contact_identifier) as event_contact_label', 'event_contact_type', 'IFNULL(event_contact_content,"") as event_contact_content', 'event_contact_default_content'));
        $srch->doNotCalculateRecords();
        $srch->doNotLimitRecords();
        $rs = $srch->getResultSet();
        $resultData = FatApp::getDb()->fetch($rs);
        if (empty($resultData['event_contact_content'])) {
            return "";
        }
        return $resultData['event_contact_content'];
    }
