...
class EventPress extends MyAppModel
{

    const DB_TBL = 'tbl_event_press';
    const DB_TBL_PREFIX = 'epage_';
    const DB_TBL_LANG = 'tbl_event_press_lang';
    const DB_TBL_LANG_PREFIX = 'epagelang_';
    const BLOCK_PRESS_INFORMATION = 26;
    ...
    public static function getSearchObject($langId = 0, $isActive = true)
    {
        ...
        if ($isActive) {
            $srch->addCondition('event_press_active', '=', applicationConstants::ACTIVE);
        }
        return $srch;
    }
    ...
    public static function getBlockContent(int $pageType, int $langId = 0): string
    {
        ...
        $srch->addCondition('ep.event_press_type', '=', $pageType);
        $srch->addMultipleFields(array('epage_id', 'IFNULL(event_press_label, event_press_identifier) as event_press_label', 'event_press_type', 'IFNULL(event_press_content,"") as event_press_content', 'event_press_default_content'));
        $srch->doNotCalculateRecords();
        $srch->doNotLimitRecords();
        $rs = $srch->getResultSet();
        $resultData = FatApp::getDb()->fetch($rs);
        if (empty($resultData['event_press_content'])) {
            return "";
        }
        return $resultData['event_press_content'];
    }
