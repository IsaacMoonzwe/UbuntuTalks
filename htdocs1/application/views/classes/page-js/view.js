...
$(function () {
    ...
    viewLessonDetail = function () {
        ...
        fcom.ajax(fcom.makeUrl('Classes', 'viewLessonDetail', [lDetailId]), '', function (t) {
            $(dv).html(t);
    ...
    scheduleLessonSetup = function (lDetailId, startTime, endTime, date) {
        fcom.ajax(fcom.makeUrl('Classes', 'scheduleLessonSetup'), 'lDetailId=' + lDetailId + '&startTime=' + startTime + '&endTime=' + endTime + '&date=' + date, function (doc) {
            $.facebox.close();
            viewLessonDetail();
        });
    };
    ...
    searchFlashCards = function (frm) {
        ...
        var data = fcom.frmData(frm);
        fcom.ajax(fcom.makeUrl('Classes', 'searchFlashCards'), data, function (t) {
            $('#flashCardListing').html(t);
        });
    };

    viewFlashCard = function (flashcardId) {
        fcom.ajax(fcom.makeUrl('Classes', 'viewFlashCard', [flashcardId]), '', function (t) {
            try {
    ...
    flashCardForm = function (lessonId, flashcardId) {
        fcom.ajax(fcom.makeUrl('Classes', 'flashCardForm'), 'flashcardId=' + flashcardId + '&lessonId=' + lessonId, function (t) {
            try {
                var res = JSON.parse(t)
                console.log(res);
                if (res.status == 0) {
        ...
        $.confirm({
            ...
            buttons: {
                Proceed: {
                    ...
                    keys: ['enter', 'shift'],
                    action: function () {
                        fcom.updateWithAjax(fcom.makeUrl('Classes', 'removeFlashcard', [id]), '', function (t) {
                            searchFlashCards(frmFlashCardSrch);
                        });
                    }
                },
    ...
    setupFlashCard = function (frm) {
        ...
        var data = fcom.frmData(frm);
        fcom.updateWithAjax(fcom.makeUrl('Classes', 'setupFlashCard'), data, function (t) {
            searchFlashCards(frmFlashCardSrch);
            $.facebox.close();
            if (t.status == 0) {
    ...
    createCometChatBox = function () {
        ...
        chat_js.onload = function () {
            ...
            chat_iframe.src = '//' + chat_appid + '.cometondemand.net/cometchat_embedded.php' + (is_grpcls == '1' ? '?guid=' + chat_group_id : '');
    ...
    joinLesson = function (learnerId, teacherId, joinFromApp) {
        $.loader.show();
        fcom.ajax(fcom.makeUrl('Classes', 'startLesson'), 'lDetailId=' + lDetailId, function (t) {
            var ans = $.parseJSON(t);
            if (ans.status) {
    ...
    endLessonSetup = function (lDetailId) {
        fcom.updateWithAjax(fcom.makeUrl('Classes', 'endLessonSetup'), 'lDetailId=' + lDetailId, function (t) {
    ...
    assignLessonPlanToLessons = function (lDetailId, planId) {
        fcom.updateWithAjax(fcom.makeUrl('Classes', 'assignLessonPlanToLessons'), 'ltp_slDetailId=' + lDetailId + '&ltp_tlpn_id=' + planId, function (t) {
            $.facebox.close();
            viewLessonDetail();
        });
    };
    ...
    removeAssignedLessonPlan = function (lDetailId) {
        if (confirm(langLbl.confirmRemove)) {
            fcom.updateWithAjax(fcom.makeUrl('Classes', 'removeAssignedLessonPlan'), 'ltp_slDetailId=' + lDetailId, function (t) {
                $.facebox.close();
                viewLessonDetail();
            });
        }
    ...
    viewAssignedLessonPlan = function (lDetailId) {
        fcom.ajax(fcom.makeUrl('Classes', 'viewAssignedLessonPlan', [lDetailId]), '', function (t) {
            $.facebox(t, 'facebox-medium');
        });
    };
