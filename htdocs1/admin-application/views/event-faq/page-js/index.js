...
(function() {
	...
	searchFaq = function(form){		
		...
		fcom.ajax(fcom.makeUrl('EventFaq','search'),data,function(res){
	...
	FaqForm = function(id) {
		...
		//$.facebox(function() {
			fcom.ajax(fcom.makeUrl('EventFaq', 'form', [id]), '', function(t) {
	...
	editFaqForm = function(faqId){
		fcom.displayProcessing();
		//$.facebox(function() {
			fcom.ajax(fcom.makeUrl('EventFaq', 'form', [faqId]), '', function(t) {
				//$.facebox(t,'faceboxWidth');
				fcom.updateFaceboxContent(t);
			});
		//});
	...
	setupFaq = function (frm){
		...
		fcom.updateWithAjax(fcom.makeUrl('EventFaq', 'setup'), data, function(t) {	
			//$.mbsmessage.close();
			reloadList();
			if (t.langId>0) {
	...
	//	$.facebox(function() {
		console.log(faqId + ', ' + langId);
			fcom.ajax(fcom.makeUrl('EventFaq', 'langForm', [faqId,langId]), '', function(t) {
	...
	setupLangFaq = function (frm){
		if (!$(frm).validate()) return;		
		var data = fcom.frmData(frm);
		fcom.updateWithAjax(fcom.makeUrl('EventFaq', 'langSetup'), data, function(t) {
			reloadList();			
			if (t.langId>0) {
				editFaqLangForm(t.faqId, t.langId);
				return ;
	...
	deleteRecord = function(id){
		...
		data='faqId='+id;
		console.log(data);
		fcom.updateWithAjax(fcom.makeUrl('EventFaq', 'deleteRecord'),data,function(res){	
	...
	activeStatus = function(obj){
		...
		fcom.ajax(fcom.makeUrl('EventFaq','changeStatus'),data,function(res){
