...
(function() {
	...
	searchTestimonial = function(frm){
		...
		fcom.ajax(fcom.makeUrl('OurVision','search'),data,function(res){
			$(dv).html(res);			
		});
	};
	...
	testimonialForm = function(id) {
		...
		//$.facebox(function() {
			fcom.ajax(fcom.makeUrl('OurVision', 'form', [id]), '', function(t) {
	...
	editTestimonialForm = function(testimonialId){
		fcom.displayProcessing();
		//$.facebox(function() {
			fcom.ajax(fcom.makeUrl('OurVision', 'form', [testimonialId]), '', function(t) {
				//$.facebox(t,'faceboxWidth');
				fcom.updateFaceboxContent(t);
			});
		//});
	...
	setupTestimonial = function (frm){
		...
		fcom.updateWithAjax(fcom.makeUrl('OurVision', 'setup'), data, function(t) {			
			//$.mbsmessage.close();
			reloadList();
			if (t.langId>0) {
	...
	editTestimonialLangForm = function(testimonialId,langId){
		fcom.displayProcessing();
	//	$.facebox(function() {
			fcom.ajax(fcom.makeUrl('OurVision', 'langForm', [testimonialId,langId]), '', function(t) {
				//$.facebox(t,'faceboxWidth');
				fcom.updateFaceboxContent(t);
			});
		//});
	...
	setupLangTestimonial = function (frm){
		if (!$(frm).validate()) return;		
		var data = fcom.frmData(frm);
		fcom.updateWithAjax(fcom.makeUrl('OurVision', 'langSetup'), data, function(t) {
			reloadList();			
			if (t.langId>0) {
				editTestimonialLangForm(t.testimonialId, t.langId);
				return ;
	...
	deleteRecord = function(id){
		...
		fcom.updateWithAjax(fcom.makeUrl('OurVision', 'deleteRecord'),data,function(res){		
			reloadList();
		});
	};
	...
	activeStatus = function(obj){
		...
		fcom.ajax(fcom.makeUrl('OurVision','changeStatus'),data,function(res){
	...
	testimonialMediaForm = function(testimonialId){
		//$.facebox(function() {
			fcom.displayProcessing();
			fcom.ajax(fcom.makeUrl('OurVision', 'media', [testimonialId]), '', function(t) {
				//$.facebox(t);
				fcom.updateFaceboxContent(t);
			});
		//});
	...
	removeTestimonialImage = function( testimonialId, langId ){
		...
		fcom.updateWithAjax(fcom.makeUrl('OurVision', 'removeTestimonialImage',[testimonialId, langId]), '', function(t) {
...
$(document).on('click','.uploadFile-Js',function(){
	...
	timer = setInterval(function() {
		if ($('#form-upload input[name=\'file\']').val() != '') {
			...
			$val = $(node).val();			
			$.ajax({
				url: fcom.makeUrl('OurVision', 'uploadTestimonialMedia'),
				type: 'post',
				dataType: 'json',
