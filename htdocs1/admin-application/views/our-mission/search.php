...
$arr_flds = array(
    ...
    'our_mission_user_name' => Label::getLabel('LBL_our_mission_Name', $adminLangId) ,
    'our_mission_description' => Label::getLabel('LBL_Description', $adminLangId) ,
    'our_mission_active' => Label::getLabel('LBL_Status', $adminLangId) ,
...
foreach ($arr_listing as $sn => $row)
{
    ...
    $tr->setAttribute("id", $row['our_mission_id']);
    ...
    foreach ($arr_flds as $key => $val)
    {
        ...
        {
            case 'dragdrop':
                if ($row['our_mission_active'] == applicationConstants::ACTIVE) {
                    $td->appendElement('i', array('class' => 'ion-arrow-move icon'));
                    $td->setAttribute("class", 'dragHandle');
                }
                break;
            case  'testimonial_image':
                if (!empty($row['our_mission_image'])) {
                    foreach ($row['our_mission_image'] as $testimonialImg) {
                        $htmlAfterField = '<img src="' . CommonHelper::generateFullUrl('OurMission', 'image', array($testimonialImg['afile_record_id'], $testimonialImg['afile_lang_id'], 'MEDIUM')) . '?' . time() . '">';
                    }
                }
            ...
            break;
            case 'our_mission_active':
                $active = "";
                $statusAct = '';
                if ($row['our_mission_active'] == applicationConstants::YES && $canEdit === true)
                {
                    $active = 'checked';
                    $statusAct = 'inactiveStatus(this)';
                }
                if ($row['our_mission_active'] == applicationConstants::NO && $canEdit === true)
                {
                    $active = '';
                ...
                $str = '<label class="statustab -txt-uppercase">                 
                     <input ' . $active . ' type="checkbox" id="switch' . $row['our_mission_id'] . '" value="' . $row['our_mission_id'] . '" onclick="' . $statusAct . '" class="switch-labels status_' . $row['our_mission_id'] . '"/>
            ...
            case 'action':
                ...
                if ($canEdit)
                {
                    ...
                    $innerLiEdit->appendElement('a', array(
                        ...
                        'class' => 'button small green',
                        'title' => Label::getLabel('LBL_Edit', $adminLangId) ,
                        "onclick" => "editTestimonialFormNew(" . $row['our_mission_id'] . ")"
                    ...
                    $innerLiDelete->appendElement('a', array(
                        ...
                        "onclick" => "deleteRecord(" . $row['our_mission_id'] . ")"
