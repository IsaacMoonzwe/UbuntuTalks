...
lessonFeedback = function (lDetailId) {
    fcom.ajax(fcom.makeUrl('PurchasedLessons', 'lessonFeedback', [lDetailId]), '', function (t) {
...
setupLessonFeedback = function (frm) {
    ...
    fcom.updateWithAjax(fcom.makeUrl('PurchasedLessons', 'setupLessonFeedback'), data, function (t) {
        $.facebox.close();
        window.location.href = fcom.makeUrl('PurchasedLessons') + '#' + statusCompleted;
        location.reload();
    });
};

requestReschedule = function (id) {
    fcom.ajax(fcom.makeUrl('PurchasedLessons', 'requestReschedule', [id]), '', function (t) {
...
requestRescheduleSetup = function (frm) {
    if (!$(frm).validate())
        return;
    var data = fcom.frmData(frm);
    fcom.updateWithAjax(fcom.makeUrl('PurchasedLessons', 'requestRescheduleSetup'), data, function (t) {
        $.facebox.close();
        window.location.href = fcom.makeUrl('PurchasedLessons') + '#' + statusScheduled;
        location.reload();
    });
};
...
viewBookingCalendar = function (id, action = '') {
    ...
    fcom.ajax(fcom.makeUrl('PurchasedLessons', 'viewBookingCalendar', [id]), data, function (t) {
        $.mbsmessage.close();
        $.facebox(t, 'facebox-large booking-calendar-pop-js');
...
setUpLessonSchedule = function (teacherId, lDetailId, startTime, endTime, date) {
    ...
    fcom.ajax(fcom.makeUrl('PurchasedLessons', 'isSlotTaken'), 'teacherId=' + teacherId + '&startTime=' + startTime + '&endTime=' + endTime + '&date=' + date, function (t) {
        t = JSON.parse(t);
        slot = t.count;
console.log(slot);
                ...
                buttons: {
                    Proceed: {
                        ...
                        action: function () {
                            $.loader.show();
                            // $.mbsmessage(langLbl.requestProcessing, false, 'alert alert--process');
                            fcom.updateWithAjax(fcom.makeUrl('PurchasedLessons', 'setUpLessonSchedule'), ajaxData, function (doc) {
                                // window.location.href = fcom.makeUrl('PurchasedLessons') + '#' + statusScheduled;
...
cancelLesson = function (id) {
    ...
    fcom.ajax(fcom.makeUrl('PurchasedLessons', 'cancelLesson', [id]), '', function (t) {
...
cancelLessonSetup = function (frm) {
    ...
    var data = fcom.frmData(frm);
    fcom.ajax(fcom.makeUrl('PurchasedLessons', 'cancelLessonSetup'), data, function (ans) {
        isLessonCancelAjaxRun = false;
        if (ans.status != 1) {
        ...
        $.facebox.close();
        window.location.href = fcom.makeUrl('PurchasedLessons') + '#' + statusUpcoming;
        location.reload();
    }, { fOutMode: 'json' });

    // fcom.updateWithAjax(fcom.makeUrl('PurchasedLessons', 'cancelLessonSetup'), data , function(t) {
    // 		$.facebox.close();
    // 		location.reload();
    // });
};
