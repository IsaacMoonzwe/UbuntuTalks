...
(function() {
	...
	searchBlocks = function(form){
		...
		fcom.ajax(fcom.makeUrl('EventPress','search'),data,function(res){
			$(dv).html(res);
		});
	};
	...
	addBlockForm = function(id) {
		...
		var frm = document.frmBlockSearch;			
			fcom.ajax(fcom.makeUrl('EventPress', 'form', [id]), '', function(t) {
	...
	setupBlock = function(frm) {
		...
		fcom.updateWithAjax(fcom.makeUrl('EventPress', 'setup'), data, function(t) {
			if ( t.langId > 0 ) {
	...
	addBlockLangForm = function(epageId, langId){
		...
		fcom.resetEditorInstance();
			fcom.ajax(fcom.makeUrl('EventPress', 'langForm', [epageId, langId]), '', function(t) {
				...
				$(frm).submit(function(e) {
					...
					var data = fcom.frmData(frm);
					fcom.updateWithAjax(fcom.makeUrl('EventPress', 'langSetup'), data, function(t) {
						fcom.resetEditorInstance();	
						reloadList();				
						if (t.langId>0) {
							addBlockLangForm(t.epageId, t.langId);
	...
	activeStatus = function(obj){
		...
		fcom.ajax(fcom.makeUrl('EventPress','changeStatus'),data,function(res){
	...
	removeBgImage = function( epage_id, langId, file_type ){
		...
		fcom.updateWithAjax( fcom.makeUrl('EventPress', 'removeBgImage',[epage_id, langId, file_type]), '', function(t) {
