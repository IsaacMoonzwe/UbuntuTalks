...
(function() {
	...
	searchBlocks = function(form){
		...
		fcom.ajax(fcom.makeUrl('ProgramAndSpeakers','search'),data,function(res){
			$(dv).html(res);
		});
	};
	...
	addBlockForm = function(id) {
		...
		var frm = document.frmBlockSearch;			
			fcom.ajax(fcom.makeUrl('ProgramAndSpeakers', 'form', [id]), '', function(t) {
	...
	setupBlock = function(frm) {
		...
		fcom.updateWithAjax(fcom.makeUrl('ProgramAndSpeakers', 'setup'), data, function(t) {
			if ( t.langId > 0 ) {
	...
	addBlockLangForm = function(epageId, langId){
		...
		fcom.resetEditorInstance();
			fcom.ajax(fcom.makeUrl('ProgramAndSpeakers', 'langForm', [epageId, langId]), '', function(t) {
				...
				$(frm).submit(function(e) {
					...
					var data = fcom.frmData(frm);
					fcom.updateWithAjax(fcom.makeUrl('ProgramAndSpeakers', 'langSetup'), data, function(t) {
						fcom.resetEditorInstance();	
						reloadList();				
						if (t.langId>0) {
							addBlockLangForm(t.epageId, t.langId);
	...
	activeStatus = function(obj){
		...
		fcom.ajax(fcom.makeUrl('ProgramAndSpeakers','changeStatus'),data,function(res){
	...
	removeBgImage = function( epage_id, langId, file_type ){
		...
		fcom.updateWithAjax( fcom.makeUrl('ProgramAndSpeakers', 'removeBgImage',[epage_id, langId, file_type]), '', function(t) {
