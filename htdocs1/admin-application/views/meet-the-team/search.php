...
$arr_flds = array(
    ...
    'meet_the_team_user_name' => Label::getLabel('LBL_meet_the_team_Name', $adminLangId) ,
    'meet_the_team_positions' => Label::getLabel('LBL_meet_the_team_Position', $adminLangId) ,
    'meet_the_team_description' => Label::getLabel('LBL_Description', $adminLangId) ,
    'meet_the_team_active' => Label::getLabel('LBL_Status', $adminLangId) ,
...
foreach ($arr_listing as $sn => $row)
{
    ...
    $tr->setAttribute("id", $row['meet_the_team_id']);
    ...
    foreach ($arr_flds as $key => $val)
    {
        ...
        {
            case 'dragdrop':
                if ($row['meet_the_team_active'] == applicationConstants::ACTIVE) {
                    $td->appendElement('i', array('class' => 'ion-arrow-move icon'));
                    $td->setAttribute("class", 'dragHandle');
                }
                break;
            case  'testimonial_image':
                if (!empty($row['meet_the_team_image'])) {
                    foreach ($row['meet_the_team_image'] as $testimonialImg) {
                        $htmlAfterField = '<img src="' . CommonHelper::generateFullUrl('MeetTheTeam', 'image', array($testimonialImg['afile_record_id'], $testimonialImg['afile_lang_id'], 'MEDIUM')) . '?' . time() . '">';
                    }
                }
            ...
            break;
            case 'meet_the_team_active':
                $active = "";
                $statusAct = '';
                if ($row['meet_the_team_active'] == applicationConstants::YES && $canEdit === true)
                {
                    $active = 'checked';
                    $statusAct = 'inactiveStatus(this)';
                }
                if ($row['meet_the_team_active'] == applicationConstants::NO && $canEdit === true)
                {
                    $active = '';
                ...
                $str = '<label class="statustab -txt-uppercase">                 
                     <input ' . $active . ' type="checkbox" id="switch' . $row['meet_the_team_id'] . '" value="' . $row['meet_the_team_id'] . '" onclick="' . $statusAct . '" class="switch-labels status_' . $row['meet_the_team_id'] . '"/>
            ...
            case 'action':
                ...
                if ($canEdit)
                {
                    ...
                    $innerLiEdit->appendElement('a', array(
                        ...
                        'class' => 'button small green',
                        'title' => Label::getLabel('LBL_Edit', $adminLangId) ,
                        "onclick" => "editTestimonialFormNew(" . $row['meet_the_team_id'] . ")"
                    ...
                    $innerLiDelete->appendElement('a', array(
                        ...
                        "onclick" => "deleteRecord(" . $row['meet_the_team_id'] . ")"
    ...
    $(document).ready(function () {
        $('#paymentMethod').tableDnD({
            onDrop: function (table, row) {
                ...
                fcom.ajax(fcom.makeUrl('OurMission', 'updateOrder'), order, function (res) {
