...
$arr_flds = array(
    ...
    'pre_symposium_dinner_plan_title' => Label::getLabel('LBL_Pre_Symposium_Dinner_Plan_Title', $adminLangId) ,
    'pre_symposium_dinner_plan_price' => Label::getLabel('LBL_Price', $adminLangId) ,
    // 'pre_symposium_dinner_starting_days' => Label::getLabel('LBL_pre_symposium_dinner_Starting_Days', $adminLangId) ,
    'pre_symposium_dinner_plan_description' => Label::getLabel('LBL_Description', $adminLangId) ,
    'pre_symposium_dinner_plan_note' => Label::getLabel('LBL_Note', $adminLangId) ,
    //'event_agenda_schedule' => Label::getLabel('LBL_View_Agenda_Schedule', $adminLangId) ,
    'pre_symposium_dinner_active' => Label::getLabel('LBL_Status', $adminLangId) ,
...
foreach ($arr_listing as $sn => $row)
{
    ...
    $tr->setAttribute("id", $row['pre_symposium_dinner_id']);
    ...
    foreach ($arr_flds as $key => $val)
    {
        ...
        {
            case 'dragdrop':
                if ($row['pre_symposium_dinner_active'] == applicationConstants::ACTIVE) {
            ...
            case 'event_agenda_schedule':
                ...
                if ($canEdit)
                {
                    ...
                    $li->appendElement('a', array(
                        ...
                        "onclick" => "editTestimonialAgendaFormNew(" . $row['pre_symposium_dinner_id'] . ")",
            ...
            break;
            case 'pre_symposium_dinner_active':
                $active = "";
                $statusAct = '';
                if ($row['pre_symposium_dinner_active'] == applicationConstants::YES && $canEdit === true)
                {
                    $active = 'checked';
                    $statusAct = 'inactiveStatus(this)';
                }
                if ($row['pre_symposium_dinner_active'] == applicationConstants::NO && $canEdit === true)
                {
                    $active = '';
                ...
                $str = '<label class="statustab -txt-uppercase">                 
                     <input ' . $active . ' type="checkbox" id="switch' . $row['pre_symposium_dinner_id'] . '" value="' . $row['pre_symposium_dinner_id'] . '" onclick="' . $statusAct . '" class="switch-labels status_' . $row['pre_symposium_dinner_id'] . '"/>
            ...
            case 'action':
                ...
                if ($canEdit)
                {
                    ...
                    $innerLiEdit->appendElement('a', array(
                        ...
                        'class' => 'button small green',
                        'title' => Label::getLabel('LBL_Edit', $adminLangId) ,
                        "onclick" => "editTestimonialFormNew(" . $row['pre_symposium_dinner_id'] . ")"
                    ...
                    $innerLiDelete->appendElement('a', array(
                        ...
                        "onclick" => "deleteRecord(" . $row['pre_symposium_dinner_id'] . ")"
    ...
    $(document).ready(function () {
        $('#paymentMethod').tableDnD({
            onDrop: function (table, row) {
                ...
                fcom.ajax(fcom.makeUrl('PreSymposiumDinner', 'updateOrder'), order, function (res) {
