...
$arr_flds = array(
    ...
    'user_id' => Label::getLabel('LBL_Visitor_Id', $adminLangId),
    'user_first_name' => Label::getLabel('LBL_Visitor_Name', $adminLangId),
    'user_email' => Label::getLabel('LBL_Visitor_Email', $adminLangId),
    'events_report_comments_information' => Label::getLabel('LBL_Report_Information', $adminLangId),
...
foreach ($arr_listing as $sn => $row) {
    if (!empty($row['total_record'])) {
        $comment_status = 0;
        if ($row['count_status'] == $row['total_record']) {
            $comment_status = 1;
        }
        ...
        foreach ($arr_flds as $key => $val) {
            ...
            switch ($key) {
                ...
                case 'status':
                    $status = ($comment_status == 1) ? Label::getLabel('LBL_COMPLETED') : Label::getLabel('LBL_PENDING');
                ...
                case 'email_reply':
                    ...
                    $li->appendElement('a', array('href' => 'javascript:void(0)', 'class' => 'button small green', 'title' => 'Report Detail', 'onclick' => 'reportDetail(' . $row['user_id'] . ');'), '<i class="ion-eye icon"></i>', true);
                    break;
    ...
    $(document).ready(function() {
        $("td:nth-child(6)").each(function() {
            ...
            if ($this.text() == 'Pending') {
                $this.parent('tr').addClass('pending-class');
            } else if ($this.text() == 'Completed') {
                $this.parent('tr').addClass('completed-class');
            }
        });
