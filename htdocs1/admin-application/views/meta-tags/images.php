    ...
    <ul class="grids--onethird" id="<?php if ($canEdit) { ?>sortable<?php } ?>">
        ...
        foreach ($images as $afile_id => $row) {
            ...
            <li id="<?php echo $row['afile_id']; ?>">
                <div class="logoWrap">
                    <div class="logothumb"> <img src="<?php echo CommonHelper::generateUrl('MetaTags', 'viewImage', array($row['afile_record_id'], $row['afile_lang_id'], '', false)) . '?' . time(); ?>" title="<?php echo $row['afile_name']; ?>" alt="<?php echo $row['afile_name']; ?>"> <?php if ($canEdit) { ?> <a class="deleteLink white" href="javascript:void(0);" title="Delete <?php echo $row['afile_name']; ?>" onclick="deleteImage(<?php echo $row['afile_record_id']; ?>, <?php echo $row['afile_lang_id']; ?>, );" class="delete"><i class="ion-close-round"></i></a>
                            <?php } ?>
                    </div>
                    <?php
                    ...
                    if ($row['afile_lang_id'] > 0) {
                        ...
                        ?>
                    <?php } ?>
                    <small class=""><strong> <?php echo Label::getLabel('LBL_Language', $adminLangId); ?>:</strong> <?php echo $lang_name; ?></small>
                </div>
            </li>
            <?php
            $count++;
