...
(function() {
	...
	searchFaq = function(form){		
		...
		fcom.ajax(fcom.makeUrl('LearnerFaq','search'),data,function(res){
	...
	FaqForm = function(id) {
		...
		//$.facebox(function() {
			fcom.ajax(fcom.makeUrl('LearnerFaq', 'form', [id]), '', function(t) {
	...
	editFaqForm = function(faqId){
		fcom.displayProcessing();
		//$.facebox(function() {
			fcom.ajax(fcom.makeUrl('LearnerFaq', 'form', [faqId]), '', function(t) {
				//$.facebox(t,'faceboxWidth');
				fcom.updateFaceboxContent(t);
			});
		//});
	...
	setupFaq = function (frm){
		...
		fcom.updateWithAjax(fcom.makeUrl('LearnerFaq', 'setup'), data, function(t) {	
			//$.mbsmessage.close();
			reloadList();
			if (t.langId>0) {
	...
	//	$.facebox(function() {
		console.log(faqId + ', ' + langId);
			fcom.ajax(fcom.makeUrl('LearnerFaq', 'langForm', [faqId,langId]), '', function(t) {
	...
	setupLangFaq = function (frm){
		if (!$(frm).validate()) return;		
		var data = fcom.frmData(frm);
		fcom.updateWithAjax(fcom.makeUrl('LearnerFaq', 'langSetup'), data, function(t) {
			reloadList();			
			if (t.langId>0) {
				editFaqLangForm(t.faqId, t.langId);
				return ;
	...
	deleteRecord = function(id){
		...
		data='faqId='+id;
		console.log(data);
		fcom.updateWithAjax(fcom.makeUrl('LearnerFaq', 'deleteRecord'),data,function(res){	
	...
	activeStatus = function(obj){
		...
		fcom.ajax(fcom.makeUrl('LearnerFaq','changeStatus'),data,function(res){
