    ...
    <ul class="grids--onethird" id="<?php if ($canEdit) { ?>sortable<?php } ?>">
        ...
        foreach ($images as $afile_id => $row) {
            ...
            <li id="<?php echo $row['afile_id']; ?>">
                <div class="logoWrap">
                    <div class="logothumb"> <img src="<?php echo CommonHelper::generateUrl('SpokenLanguage', 'Thumb', array($row['afile_record_id'], AttachedFile::FILETYPE_SPOKEN_LANGUAGES, $row['afile_lang_id'], $row['afile_screen'])) . '?' . time(); ?>" title="<?php echo $row['afile_name']; ?>" alt="<?php echo $row['afile_name']; ?>"> <?php if ($canEdit) { ?> <a class="deleteLink white" href="javascript:void(0);" title="Delete <?php echo $row['afile_name']; ?>" onclick="removeImage(<?php echo $slanguage_id; ?>, <?php echo $row['afile_record_id']; ?>, <?php echo $row['afile_lang_id']; ?>, <?php echo $row['afile_screen']; ?>);" class="delete"><i class="ion-close-round"></i></a>
                            <?php } ?>
                    </div>
                    <?php
