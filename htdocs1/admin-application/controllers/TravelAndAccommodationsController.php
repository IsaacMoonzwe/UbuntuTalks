...
class TravelAndAccommodationsController extends AdminBaseController
{
    ...
    {
        $srch = TravelAndAccommodations::getSearchObject($this->adminLangId, false);
        $srch->addOrder('travel_and_accommodations_active', 'DESC');
    ...
    public function form($epage_id = 0)
    {
        ...
        if (0 < $epage_id) {
            $data = TravelAndAccommodations::getAttributesById($epage_id, array('epage_id', 'travel_and_accommodations_identifier', 'travel_and_accommodations_active'));
            if ($data === false) {
    ...
    public function setup()
    {
        ...
        $extraPage = new TravelAndAccommodations($epage_id);
        ...
        foreach ($languages as $langId => $langName) {
            if (!$row = TravelAndAccommodations::getAttributesByLangId($langId, $epage_id)) {
                $newTabLangId = $langId;
                break;
            }
        }
    ...
    public function langForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $epageData = TravelAndAccommodations::getAttributesById($epage_id);
        $blockLangFrm = $this->getLangForm($epage_id, $lang_id);
        $langData = TravelAndAccommodations::getAttributesByLangId($lang_id, $epage_id);
        if ($langData) {
    ...
    public function langSetup()
    {
        ...
        $data = array(
            ...
            'travel_and_accommodations_label' => $post['travel_and_accommodations_label'],
            'travel_and_accommodations_content' => $post['travel_and_accommodations_content'],
        );
        $epageObj = new TravelAndAccommodations($epage_id);
    ...
    public function changeStatus()
    {
        ...
        $contentBlockData = TravelAndAccommodations::getAttributesById($epageId, array('travel_and_accommodations_active'));
        if (!$contentBlockData) {
            Message::addErrorMessage($this->str_invalid_request);
            FatUtility::dieWithError(Message::getHtml());
        }
        $status = ($contentBlockData['travel_and_accommodations_active'] == applicationConstants::ACTIVE) ? applicationConstants::INACTIVE : applicationConstants::ACTIVE;
    ...
    private function updateEPageStatus($epageId, $status)
    {
        ...
        $EPageObj = new TravelAndAccommodations($epageId);
    ...
    private function getForm($epage_id = 0, $langId = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Identifier', $this->adminLangId), 'travel_and_accommodations_identifier');

        $activeInactiveArr = applicationConstants::getActiveInactiveArr($langId);
        $frm->addSelectBox(Label::getLabel('LBL_Status', $this->adminLangId), 'travel_and_accommodations_active', $activeInactiveArr, '', array(), '');
    ...
    private function getLangForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Title', $this->adminLangId), 'travel_and_accommodations_label');
        $frm->addHtmlEditor(Label::getLabel('LBL_Page_Content', $this->adminLangId), 'travel_and_accommodations_content');
