...
class FreeDaysController extends AdminBaseController
{
    ...
    {
        $srch = FreeDays::getSearchObject($this->adminLangId, false);
    ...
    public function form($faqId)
    {
        ...
        if (0 < $faqId) {
            $data = FreeDays::getAttributesById($faqId, ['faq_id', 'faq_identifier', 'faq_category', 'faq_active']);
            if ($data === false) {
    ...
    public function setup()
    {
        ...
        if ($faqId > 0) {
            ...
            foreach ($languages as $langId => $langName) {
                if (!$row = FreeDays::getAttributesByLangId($langId, $faqId)) {
                    $newTabLangId = $langId;
                    break;
                }
            }
    ...
    public function langForm($faqId = 0, $lang_id = 0)
    {
        ...
        $langData = FreeDays::getAttributesByLangId($lang_id, $faqId);
        if ($langData) {
    ...
    public function changeStatus()
    {
        ...
        $data = FreeDays::getAttributesById($faqId, ['faq_id', 'faq_active']);
        if ($data == false) {
    ...
    public function deleteRecord()
    {
        ...
        $LessonPackageObj = new FreeDays($faq_id);
    ...
    private function getForm($faqId = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Free_Days_Trials', $this->adminLangId), 'faq_identifier');
        // $fld = $frm->addSelectBox(Label::getLabel('LBL_faq_category', $this->adminLangId), 'faq_category', FreeDays::getFaqCategoryArr($this->adminLangId));
    ...
    private function getLangForm($faqId = 0, $lang_id = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Free_Days_Trials', $this->adminLangId), 'faq_title');
        $frm->addHtmlEditor(Label::getLabel('LBL_Free_Days_Trials_Description', $this->adminLangId), 'faq_description');
        $frm->addSubmitButton('', 'btn_submit', Label::getLabel('LBL_Save_Changes', $this->adminLangId));
        return $frm;
    }
