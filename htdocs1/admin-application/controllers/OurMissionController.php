...
class OurMissionController extends AdminBaseController
{
    ...
    public function search()
    {
        ...
        $srch = OurMission::getSearchObject($this->adminLangId, false);
        $srch->addMultipleFields(['t.*', 't_l.our_mission_title', 't_l.our_mission_text']);
        $srch->addOrder('our_mission_active', 'desc');
    ...
    public function form($testimonialId)
    {
        ...
        if (0 < $testimonialId) {
            $data = OurMission::getAttributesById($testimonialId, [
                'our_mission_id',
                'our_mission_identifier',
                'our_mission_active',
                'our_mission_user_name',
                'our_mission_positions',
                'our_mission_description',
                'our_mission_positions_listing'
            ]);
            if ($data === false) {
        ...
        $this->set('our_mission_id', $testimonialId);
    ...
    public function setup()
    {
        ...
        $testimonialId = $post['our_mission_id'];
        unset($post['our_mission_id']);
        if ($testimonialId == 0) {
            $post['our_mission_added_on'] = date('Y-m-d H:i:s');
        }
        $record = new OurMission($testimonialId);
        ...
        if ($testimonialId > 0) {
            ...
            foreach ($languages as $langId => $langName) {
                // if (!$row = OurMission::getAttributesByLangId($langId, $testimonialId)) {
                //     $newTabLangId = $langId;
                //     break;
                // }
            }
    ...
    public function langForm($testimonialId = 0, $lang_id = 0)
    {
        ...
        $langData = OurMission::getAttributesByLangId($lang_id, $testimonialId);
        if ($langData) {
    ...
    public function langSetup()
    {
        ...
        $data = [
            ...
            'testimoniallang_our_mission_id' => $testimonialId,
            'our_mission_title' => $post['our_mission_title'],
            'our_mission_text' => $post['our_mission_text']
        ];
        $obj = new OurMission($testimonialId);
    ...
    public function changeStatus()
    {
        ...
        $data = OurMission::getAttributesById($testimonialId, ['our_mission_id', 'our_mission_active']);
        if ($data == false) {
            Message::addErrorMessage($this->str_invalid_request);
            FatUtility::dieWithError(Message::getHtml());
        }
        $status = ($data['our_mission_active'] == applicationConstants::ACTIVE) ? applicationConstants::INACTIVE : applicationConstants::ACTIVE;
        $obj = new OurMission($testimonialId);
    ...
    public function deleteRecord()
    {
        ...
        $testimonialObj = new OurMission($testimonial_id);
        if (!$testimonialObj->canRecordMarkDelete($testimonial_id)) {
            Message::addErrorMessage($this->str_invalid_request_id);
            FatUtility::dieJsonError(Message::getHtml());
        }
        $testimonialObj->assignValues([OurMission::tblFld('deleted') => 1]);
    ...
    public function media($testimonialId = 0)
    {
        ...
        $testimonialImages = AttachedFile::getMultipleAttachments(AttachedFile::FILETYPE_OUR_MISSION_IMAGE, $testimonialId, 0, -1);
        $this->set('languages', Language::getAllNames());
        $this->set('testimonialId', $testimonialId);
    ...
    {
        $frm = new Form('frmOurMissionMedia');
        $frm->addButton(Label::getLabel('Lbl_Image', $this->adminLangId), 'testimonial_image', Label::getLabel('LBL_Upload_Image', $this->adminLangId), [
            ...
            'data-file_type' => AttachedFile::FILETYPE_OUR_MISSION_IMAGE,
    ...
    public function uploadTestimonialMedia()
    {
        ...
        $fileHandlerObj->deleteFile($fileHandlerObj::FILETYPE_OUR_MISSION_IMAGE, $testimonialId, 0, 0, $lang_id);
        if (!$res = $fileHandlerObj->saveAttachment($_FILES['file']['tmp_name'], $fileHandlerObj::FILETYPE_OUR_MISSION_IMAGE, $testimonialId, 0, $_FILES['file']['name'], -1, $unique_record = false, $lang_id)) {
    ...
    public function removeTestimonialImage($testimonialId = 0, $lang_id = 0)
    {
        ...
        $fileHandlerObj = new AttachedFile();
        if (!$fileHandlerObj->deleteFile(AttachedFile::FILETYPE_OUR_MISSION_IMAGE, $testimonialId, 0, 0, $lang_id)) {
            Message::addErrorMessage($fileHandlerObj->getError());
            FatUtility::dieJsonError(Message::getHtml());
        }
    ...
    {
        $testimonialId = FatUtility::int($testimonialId);
        $frm = new Form('frmOurMission');
        $frm->addHiddenField('', 'our_mission_id', $testimonialId);
        $frm->addHtml('', 'testimonial_image_display_div', '');
        $frm->addRequiredField(Label::getLabel('LBL_our_mission_Name', $this->adminLangId), 'our_mission_user_name');
        $frm->addTextarea(Label::getLabel('LBL_Description', $this->adminLangId), 'our_mission_description')->requirements()->setRequired();
        $activeInactiveArr = applicationConstants::getActiveInactiveArr($this->adminLangId);
        $frm->addSelectBox(Label::getLabel('LBL_Status', $this->adminLangId), 'our_mission_active', $activeInactiveArr, '', [], '');
    ...
    private function getLangForm($testimonialId = 0, $lang_id = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Testimonial_Title', $this->adminLangId), 'our_mission_title');
        $frm->addTextarea(Label::getLabel('LBL_Testimonial_Text', $this->adminLangId), 'our_mission_text');
        $frm->addSubmitButton('', 'btn_submit', Label::getLabel('LBL_Save_Changes', $this->adminLangId));
        return $frm;
    }
    ...
    {
        if ($attachment = AttachedFile::getAttachment(AttachedFile::FILETYPE_OUR_MISSION_IMAGE, $testimonialId, 0)) {
            return true;
        }
        return false;
    }
    ...
    public function image($recordId, $langId = 0, $sizeType = '', $afile_id = 0, $displayUniversalImage = true)
    {
        ...
        if ($afile_id > 0) {
            ...
            if (!false == $res && $res['afile_type'] == AttachedFile::FILETYPE_OUR_MISSION_IMAGE) {
                $file_row = $res;
            }
        } else {
