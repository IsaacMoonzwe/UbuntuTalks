...
class ProgramAndSpeakersController extends AdminBaseController
{
    ...
    {
        $srch = ProgramAndSpeakers::getSearchObject($this->adminLangId, false);
        $srch->addOrder('program_speakers_active', 'DESC');
    ...
    public function form($epage_id = 0)
    {
        ...
        if (0 < $epage_id) {
            $data = ProgramAndSpeakers::getAttributesById($epage_id, array('epage_id', 'program_speakers_identifier', 'program_speakers_active'));
            if ($data === false) {
    ...
    public function setup()
    {
        ...
        $extraPage = new ProgramAndSpeakers($epage_id);
        ...
        foreach ($languages as $langId => $langName) {
            if (!$row = ProgramAndSpeakers::getAttributesByLangId($langId, $epage_id)) {
                $newTabLangId = $langId;
                break;
            }
        }
    ...
    public function langForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $epageData = ProgramAndSpeakers::getAttributesById($epage_id);
        $blockLangFrm = $this->getLangForm($epage_id, $lang_id);
        $langData = ProgramAndSpeakers::getAttributesByLangId($lang_id, $epage_id);
        if ($langData) {
    ...
    public function langSetup()
    {
        ...
        $data = array(
            ...
            'program_speakers_label' => $post['program_speakers_label'],
            'program_speakers_content' => $post['program_speakers_content'],
        );
        $epageObj = new ProgramAndSpeakers($epage_id);
    ...
    public function changeStatus()
    {
        ...
        $contentBlockData = ProgramAndSpeakers::getAttributesById($epageId, array('program_speakers_active'));
        if (!$contentBlockData) {
            Message::addErrorMessage($this->str_invalid_request);
            FatUtility::dieWithError(Message::getHtml());
        }
        $status = ($contentBlockData['program_speakers_active'] == applicationConstants::ACTIVE) ? applicationConstants::INACTIVE : applicationConstants::ACTIVE;
    ...
    private function updateEPageStatus($epageId, $status)
    {
        ...
        $EPageObj = new ProgramAndSpeakers($epageId);
    ...
    private function getForm($epage_id = 0, $langId = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Identifier', $this->adminLangId), 'program_speakers_identifier');

        $activeInactiveArr = applicationConstants::getActiveInactiveArr($langId);
        $frm->addSelectBox(Label::getLabel('LBL_Status', $this->adminLangId), 'program_speakers_active', $activeInactiveArr, '', array(), '');
    ...
    private function getLangForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Title', $this->adminLangId), 'program_speakers_label');
        $frm->addHtmlEditor(Label::getLabel('LBL_Page_Content', $this->adminLangId), 'program_speakers_content');
