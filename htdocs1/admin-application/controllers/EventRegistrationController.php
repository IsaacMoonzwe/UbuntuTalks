...
class EventRegistrationController extends AdminBaseController
{
    ...
    {
        $srch = EventRegistration::getSearchObject($this->adminLangId, false);
        $srch->addOrder('event_registration_active', 'DESC');
    ...
    public function form($epage_id = 0)
    {
        ...
        if (0 < $epage_id) {
            $data = EventRegistration::getAttributesById($epage_id, array('epage_id', 'event_registration_identifier', 'event_registration_active'));
            if ($data === false) {
    ...
    public function setup()
    {
        ...
        $extraPage = new EventRegistration($epage_id);
        ...
        foreach ($languages as $langId => $langName) {
            if (!$row = EventRegistration::getAttributesByLangId($langId, $epage_id)) {
                $newTabLangId = $langId;
                break;
            }
        }
    ...
    public function langForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $epageData = EventRegistration::getAttributesById($epage_id);
        $blockLangFrm = $this->getLangForm($epage_id, $lang_id);
        $langData = EventRegistration::getAttributesByLangId($lang_id, $epage_id);
        if ($langData) {
    ...
    public function langSetup()
    {
        ...
        $data = array(
            ...
            'event_registration_label' => $post['event_registration_label'],
            'event_registration_content' => $post['event_registration_content'],
        );
        $epageObj = new EventRegistration($epage_id);
    ...
    public function changeStatus()
    {
        ...
        $contentBlockData = EventRegistration::getAttributesById($epageId, array('event_registration_active'));
        if (!$contentBlockData) {
            Message::addErrorMessage($this->str_invalid_request);
            FatUtility::dieWithError(Message::getHtml());
        }
        $status = ($contentBlockData['event_registration_active'] == applicationConstants::ACTIVE) ? applicationConstants::INACTIVE : applicationConstants::ACTIVE;
    ...
    private function updateEPageStatus($epageId, $status)
    {
        ...
        $EPageObj = new EventRegistration($epageId);
    ...
    private function getForm($epage_id = 0, $langId = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Identifier', $this->adminLangId), 'event_registration_identifier');

        $activeInactiveArr = applicationConstants::getActiveInactiveArr($langId);
        $frm->addSelectBox(Label::getLabel('LBL_Status', $this->adminLangId), 'event_registration_active', $activeInactiveArr, '', array(), '');
    ...
    private function getLangForm($epage_id = 0, $lang_id = 0)
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Title', $this->adminLangId), 'event_registration_label');
        $frm->addHtmlEditor(Label::getLabel('LBL_Page_Content', $this->adminLangId), 'event_registration_content');
