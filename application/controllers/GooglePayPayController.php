...
class GooglePayPayController extends PaymentController
{
    ...
    private function getPaymentForm($orderId)
    {
        ...
        $frm->addHiddenField('', 'notify_url', CommonHelper::generateNoAuthUrl('googlePayPay', 'payment'));
        ...
        if (!$orderInfo['order_id']) {
            ...
            try {
                $session = \Stripe\Checkout\Session::create([
                    ...
                    'mode' => 'payment',
                    'success_url' => CommonHelper::generateFullUrl('GooglePay', 'callback') . "?session_id={CHECKOUT_SESSION_ID}",
    ...
    if (strtolower($session->payment_status) == 'paid' && $totalPaidMatch) {
        ...
        $orderPaymentObj->addOrderPayment('GooglePay', $_GET['session_id'], $paymentGatewayCharge, 'Received Payment', serialize($session));
    } else {
