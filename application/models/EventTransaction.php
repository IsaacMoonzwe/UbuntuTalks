...
class EventTransaction extends MyAppModel
{
    ...
    public function getAttributesWithUserInfo($userId = 0, $attr = null)
    {
        ...
        $srch = static::getSearchObject();
        $srch->joinTable(EventUser::DB_TBL, 'LEFT OUTER JOIN', 'u.user_id = utxn.utxn_user_id', 'u');
        $srch->joinTable(EventUser::DB_TBL_CRED, 'LEFT OUTER JOIN', 'c.credential_user_id = u.user_id', 'c');
        if (null != $attr) {
            if (is_array($attr)) {
    ...
    public static function transactionDetailsWithLesson($lessonId)
    {
        ...
        $srch->addCondition('utxn_user_id', '=', EventUserAuthentication::getLoggedUserId());
    ...
    {
        $db = FatApp::getDb();
        if (!$db->updateFromArray(self::DB_TBL, ['utxn_status' => $status], ['smt' => 'utxn_slesson_id = ?', 'vals' => [$lessonId]])) {
            return false;
        }
        return true;
    }
