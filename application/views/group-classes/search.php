...
// $sql = "INSERT INTO tbl_event_and_agenda (event_name, event_description) VALUES ('John','john@example.com')";
...
<?php 
    if (!empty($classes)) {
        $totalClasses =  $pagingArr['recordCount'];  
    }
?>
    ...
    <div style="font-size: 18px;text-align: center;font-weight: 400;color: #333;">
            <?php echo Label::getLabel('LBL_To_request_a_personalized_group_class'); ?>
            <a href="javascript:void(0);" onclick="groupClassesForm();" class="groupcontact">
                <?php echo Label::getLabel('LBL_FILL_FORM'); ?>
            </a>
        </div>
    <?php 
        $Fromhtml= FatUtility::decodeHtmlEntities($groupBanner);         
        $findIndex=strpos($Fromhtml,"Group Class(es)");
        echo substr_replace( $Fromhtml,$totalClasses." ",$findIndex,0);
    ?>
    <?php if (!empty($classes)) { ?>
        ...
        <div class="group__list">
            <div class="row">
                ...
                foreach ($classes as $class) {
                    $_SESSION = $class['grpcls_max_learner'];
                    ...
                    if ($class['is_in_class'] || ($class['grpcls_max_learner'] > 0 && $class['total_learners'] >= $class['grpcls_max_learner']) || ($class['grpcls_start_datetime'] < date('Y-m-d H:i:s', strtotime('+' . $min_booking_time . ' minutes'))) || (UserAuthentication::isUserLogged() && $class['grpcls_teacher_id'] == UserAuthentication::getLoggedUserId()) || ($class['grpcls_status'] != TeacherGroupClasses::STATUS_ACTIVE) ){
                    ...
                    ?>
                    <div class="col-sm-6 col-md-6 col-lg-4 <?php echo $divclass; ?>">
                        <div class="card card--bg color-white custom-box">
                            <div class="card__head">
                                <a href="<?php echo CommonHelper::generateUrl('GroupClasses', 'view', [CommonHelper::htmlEntitiesDecode($class['grpcls_slug'])]); ?>">
                                    <h3><?php echo $class['grpcls_title']; ?></h3>
                                </a>
                                <img src="<?php echo FatCache::getCachedUrl(CommonHelper::generateUrl('Image', 'showLanguageFlagImage', array($class['grpcls_tlanguage_id'], 'SMALL')), CONF_IMG_CACHE_TIME, '.jpg'); ?>" alt="">
                            </div>
                            ...
                            <div class="card__body">
                                ...
                                <div class="card__row">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <p style="display:none;" class="class-type"><?php echo $class['grpcls_classes_type'] ; ?></p>
                                        <div class="timer">
                                           ...
                                           <?php if ($class['is_in_class'] || ($class['grpcls_max_learner'] > 0 && $class['total_learners'] >= $class['grpcls_max_learner']) || ($class['grpcls_start_datetime'] < date('Y-m-d H:i:s', strtotime('+' . $min_booking_time . ' minutes'))) || (UserAuthentication::isUserLogged() && $class['grpcls_teacher_id'] == UserAuthentication::getLoggedUserId()) || ($class['grpcls_status'] != TeacherGroupClasses::STATUS_ACTIVE) ){ ?>
                                            <div class="timer__content">
                                                <div class="timer__controls timer-js" id="timer-<?php echo $class['grpcls_id']; ?>" data-startTime="<?php echo date('Y/m/d H:i:s', $curDateTimeUnix); ?>" data-endTime="<?php echo date('Y/m/d H:i:s', $startDateTimeUnix); ?>">
                                                </div> 
                                            </div>
                                            <?php }else{?>
                                                <div class="timer__content">
                                ...
                                <div class="card__row--action">
                                    <a href="<?php echo CommonHelper::generateUrl('GroupClasses', 'view', [CommonHelper::htmlEntitiesDecode($class['grpcls_slug'])]); ?>" class="btn btn--bordered color-primary" tabindex="0"><?php echo Label::getLabel("LBL_VIEW_DETAILS") ?></a>
                                    <?php if ($class['is_in_class']) : ?>
                                        <a href="javascript:void(0);" id=<?php echo $class['grpcls_id'];?>  title="<?php echo Label::getLabel('LBL_ALREADY_IN_CLASS') ?>" tabindex="0" class="btn btn--primary btn--disabled"><?php echo Label::getLabel("LBL_Book_Now") ?></a>
                                    <?php elseif ($class['grpcls_max_learner'] > 0 && $class['total_learners'] >= $class['grpcls_max_learner']) : ?>
                                        <a href="javascript:void(0);" id=<?php echo $class['grpcls_id'];?> title="<?php echo Label::getLabel('LBL_CLASS_FULL') ?>" tabindex="0" class="btn btn--primary btn--disabled"><?php echo Label::getLabel("LBL_Book_Now") ?></a>
                                    <?php elseif ($class['grpcls_start_datetime'] < date('Y-m-d H:i:s', strtotime('+' . $min_booking_time . ' minutes'))) : ?>
                                        <a href="javascript:void(0);" id=<?php echo $class['grpcls_id'];?> title="<?php echo Label::getLabel('LBL_Booking_Close_For_This_Class') ?>" class="btn btn--primary btn--disabled"><?php echo Label::getLabel("LBL_Book_Now") ?></a>
                                    ...
                                    <?php elseif ($class['grpcls_status'] != TeacherGroupClasses::STATUS_ACTIVE) : ?>
                                        <a href="javascript:void(0);" id=<?php echo $class['grpcls_id'];?> title="<?php echo Label::getLabel('LBL_Class_Not_active') ?>" class="btn btn--primary btn--disabled"><?php echo Label::getLabel("LBL_Book_Now") ?></a>
                                    <?php else : ?>
                                        <a href="javascript:void(0);" onclick="cart.proceedToStep({teacherId:<?php echo $class['grpcls_teacher_id']; ?>, grpclsId:<?php echo $class['grpcls_id'] ?>, languageId: <?php echo $class['grpcls_tlanguage_id'] ?>, lessonDuration: <?php echo $class['grpcls_tlanguage_id'] ?>}, 'getPaymentSummary');" id=<?php echo $class['grpcls_id'];?> class="btn btn--primary btn--medium"><?php echo Label::getLabel("LBL_Book_Now") ?></a>
                                    <?php endif; ?>
                                </div>
                                <?php if($class['grpcls_classes_type']=='Private'){ ?>
                                <div class="card__row--action">
                                    <a href="<?php echo CommonHelper::generateUrl('GroupClasses', 'view', [CommonHelper::htmlEntitiesDecode($class['grpcls_slug'])]); ?>" class="btn btn--bordered color-primary" tabindex="0"><?php echo Label::getLabel("LBL_VIEW_DETAILS") ?></a>
                                    <?php if ($class['is_in_class']) : ?>
                                        <a href="javascript:void(0);" id=<?php echo $class['grpcls_id'];?>  title="<?php echo Label::getLabel('LBL_ALREADY_IN_CLASS') ?>" tabindex="0" class="btn btn--primary btn--disabled"><?php echo Label::getLabel("LBL_Book_Now") ?></a>
                                    ...
                                    <?php elseif ($class['grpcls_status'] != TeacherGroupClasses::STATUS_ACTIVE) : ?>
                                        <a href="javascript:void(0);" id=<?php echo $class['grpcls_id'];?> title="<?php echo Label::getLabel('LBL_Class_Not_active') ?>" class="btn btn--primary btn--disabled"><?php echo Label::getLabel("LBL_Book_Now") ?></a>
                                    <?php else : ?>
                                        <a href="javascript:void(0);" onclick="cart.proceedToStep({isPrivateClass:1,teacherId:<?php echo $class['grpcls_teacher_id']; ?>, grpclsId:<?php echo $class['grpcls_id'] ?>, languageId: <?php echo $class['grpcls_tlanguage_id'] ?>, lessonDuration: <?php echo $class['grpcls_tlanguage_id'] ?>}, 'selectionBookingForm');" id=<?php echo $class['grpcls_id'];?> class="btn btn--primary btn--medium"><?php echo Label::getLabel("LBL_Book_Now") ?></a>
                                    <?php endif; ?>
                                </div>
                                <?php } ?>
                            </div>
...
<script>
    jQuery(document).ready(function () {
        function addNew(id) {
            ...
            var oldId=localStorage.getItem('grpcls_id');
            if(parseInt(oldId)!=parseInt(myArrfay[1])){
            localStorage.setItem('grpcls_id',myArrfay[1]);
            $.ajax({
                url: fcom.makeUrl('GroupClasses', 'setup'),
                type: 'POST',
                dataType: 'json',
                data:formData,
        ...
        setInterval(function(){
            $( ".countdowntimer-js" ).each(function( index ) {
                var class_type_ele=$(".class-type")[index];
                var class_type=$(class_type_ele).text();
                if(class_type!='Private')
                {
                    var priceIndex=$( ".class-price" )[index];
                    var prices=$(priceIndex).text().split('$')[1];
                    if(parseInt(prices)<=0){   
                    var counterID=$(this).attr('id').split('-')[1];
                    var runningTime=$(this).text();
                    var time1 = runningTime.split(':');
                
                    if((time1[0]=='00') )
                    {
                        if((time1[1]== '00') || (time1[1]=='00')){
                            if((time1[2]<='59')){
            ...
            var timerID=$(this).attr('id');
                var runningClassTime=$(this).text();
                var classTime = runningClassTime.split(':');
                if(classTime[0]=='00' && classTime[1]=='00' && classTime[2]=='00' && classTime[3]=='01'){
