...
<div class="box box--checkout">
    <div class="box__head">
        <?php if (1 > $cartData['grpclsId']) { ?>
            ...
            if (isset($_SESSION) && $_SESSION['fromKids'] > 0 && $oneOnOne == 0) {
            ?>
                <a href="javascript:void(0);" onclick="cart.proceedToStep({fromBack:1}, 'kidsBookingForm');" class="btn btn--bordered color-black btn--back">
                    <svg class="icon icon--back">
                        <use xlink:href="<?php echo CONF_WEBROOT_URL . 'images/sprite.yo-coach.svg#back'; ?>"></use>
                    </svg>
                    <?php echo Label::getLabel('LBL_BACK'); ?>
                </a>
            <?php } elseif ($_SESSION['fromKids'] <= 0) { ?>
                <a href="javascript:void(0);" onclick="if(cart.props.isCompany>0) {cart.proceedToStep({fromBack:1}, 'groupBookingForm');} else {cart.proceedToStep({fromBack:1}, 'selectionBookingForm');
            }" class="btn btn--bordered color-black btn--back">
                    <svg class="icon icon--back">
                        <use xlink:href="<?php echo CONF_WEBROOT_URL . 'images/sprite.yo-coach.svg#back'; ?>"></use>
                    </svg>
                    <?php echo Label::getLabel('LBL_BACK'); ?>
            ...
            <div class="row">
                ...
                <?php } else { ?>
                    <div class="col-md-6 col-xl-5 offset-xl-1">
                        ...
                        <button href="javascript:void(0);" onclick="cart.confirmOrder(0, this);" class="btn btn--primary btn--large btn--block color-white"><?php echo Label::getLabel('LBL_CONFIRM_BOOKING'); ?></button>
                        <p class="payment-note color-secondary">
                            <?php
                            $labelstr = Label::getLabel('LBL_*_All_Purchases_are_in_{default-currency-code}._Foreign_transaction_fees_might_apply,_according_to_your_bank\'s_policies');
                            echo str_replace("{default-currency-code}", CommonHelper::getSystemCurrencyData()['currency_code'], $labelstr);
