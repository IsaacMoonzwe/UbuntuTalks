...
<div class="box box--checkout">
    ...
    <div class="box__body events-box-section">
        <h3 class="tickets-title"><?php echo Label::getLabel('LBL_UT_Symposium_Select_Tickets_Plan'); ?></h3>
        <div class="selection-tabs selection--checkout selection--duration selection--onehalf">
            <?php
            foreach ($slotDurations as $duration) {
                $newStrting = explode("Sales", $duration['registration_plan_title']);
            ?>
                <label class="selection-tabs__label">
                    <input type="radio" onchange="onPlanChange(this);" class="selection-tabs__input" value="<?php echo $duration['registration_plan_title']; ?>" <?php echo ($planSelected == $duration['registration_plan_title']) ? 'checked' : ''; ?> name="lessonDuration">
                    <div class="selection-tabs__title">
                        ...
                        <div>
                            <span>
                                <?php echo sprintf($newStrting[0] . "- ($" . $duration['registration_plan_price'] . ")"); ?>
                            </span>
                        </div>
                    </div>
                </label>
    ...
    <div class="box-foot">
        ...
        <div class="box-foot__right">
            ...
            <a href="javascript:void(0);" class="btn btn--primary color-white" onclick="GetEventTickets(eventCart.props.sponsershipPlan);"><?php echo Label::getLabel('LBL_NEXT'); ?></a>
        </div>
    </div>
</div>
<script>
    ...
    function onPlanChange(plan){
        eventCart.props.sponsershipPlan =plan.value;
        eventCart.props.countOfTickets=1;
    }
</script>