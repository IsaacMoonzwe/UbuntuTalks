...
var FatEventCalendar = function (teacherId) {
    ...
    teacherId = teacherId;
        ...
        this.calDefaultConf = {
        initialView: 'dayGridWeek',  // timeGridWeek or listWeek or dayGridWeek
        //weekNumbers: true,
    ...
    this.setLocale = function (locale) {
        this.calPayDefaultConf.locale = locale;
    };
...
FatEventCalendar.prototype.WeeklyBookingCalendar = function (current_time, duration, bookingBefore) {
    ...
    var calConf = {
        ...
        select: function (arg) {
            ...
            jQuery('body #d_payment_calendar .closeon').click();
            jQuery("#loaderCalendar").show();
    ...
    var defaultConf = this.calPayDefaultConf;
    var conf = { ...defaultConf, ...calConf };

    var calendarEl = document.getElementById('d_payment_calendar');
...
FatEventCalendar.prototype.AvailaibilityCalendar = function (current_time, duration, bookingBefore, selectable) {
    ...
    var calConf = {
                ...
                events: function (fetchInfo, successCallback, failureCallback) {
                    ...
                    fcom.ajax(fcom.makeUrl('Teachers', 'getTeacherWeeklyScheduleJsonData', [fecal.teacherId]), postData, function (docs) {
                        for (i in docs) {
                            ...
                            docs[i].selectable = false;
                            if ((parseInt(docs[i].classType))) {
                                docs[i].text= 'background';
                                docs[i].selectable = true;
                                docs[i].editable = true;
                            }
                            // docs[i].editable = false;
                        }
                        successCallback(docs);
        ...
        select: function (arg) {
            ...
            var durationMS = (FullCalendar.createDuration(duration).milliseconds / 1000);
            let start = moment(arg.start);
            let end = moment(arg.start).add(durationMS, 'seconds');
            let bookingBeforeDate = moment(current_time).add(bookingBefore, 'hours');
            ...
            $.loader.show();
            checkSlotAvailabiltAjaxRun = true;
            var newEvent = { start: start.format('YYYY-MM-DD HH:mm:ss'), end: end.format('YYYY-MM-DD HH:mm:ss') };
    ...
    var defaultPayConf = this.calPayDefaultConf;
    var conf = { ...defaultConf, ...calConf };
    var confPay = { ...defaultPayConf, ...calConf };
    
    if (selectable) {
        var calendarEl = document.getElementById('d_calendarfree_trial');
        var calendarPayEl = document.getElementById('d_payment_calendarfree_trial');
    } else {
        var calendarEl = document.getElementById('d_calendar');
        var calendarPayEl = document.getElementById('d_payment_calendar');
        
    }

    if(calendarEl) {
        console.log("133jhfjk");
        var calendar = new FullCalendar.Calendar(calendarEl, conf);
        calendar.render();
    ...
    if(calendarPayEl){
        console.log("jhfjk");
    var calendarPay = new FullCalendar.Calendar(calendarPayEl, confPay);
    calendarPay.render();
    window.viewOnlyCal = calendarPay;
